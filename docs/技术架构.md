...
> 最后核对日期：2026-08-30。应用版本以根目录 `package.json` 的 `appVersion` 为准。
...
| `fundamental` | 财报事实、公司行为、业绩预告、可转债档案、发行事实与条款 | `financial_reports`、`financial_facts`、`convertible_bond_profiles`、`convertible_bond_issuance`、`convertible_bond_terms` |
| `event` | 公告文档和结构化公司/证券事件 | `documents`、`company_events`、`instrument_events`、`convertible_bond_call_events`、`convertible_bond_revision_events` |
| `analytics` | 计算指标、页面快照、可转债强赎/下修/上市表现、列表/周期/估值和市场周期结果 | `metric_values`、`analysis_snapshots`、`convertible_bond_trigger_daily`、`convertible_bond_call_latest`、`convertible_bond_revision_latest`、`convertible_bond_listing_performance`、`convertible_bond_list_metrics_daily`、`convertible_bond_valuation_daily` |
...
| 可转债下修监控（`analytics.convertible_bond_revision_latest`） | 公告同步读取巨潮/上交所/深交所并写入公告事实、`event.convertible_bond_revision_events`；`calculateConvertibleBondRevisionStatus` 按 `reset-v2` 写入 `analytics.convertible_bond_trigger_daily` | `/api/bond-revision`、可转债“下修”子页 | 仅当前交易日有行情的交易所上市公开转债；退市、未上市、定向和私募债券不进入下修范围。视图以最新交易日行情为驱动，条款、进度和公告按单券索引读取，接口在数据库完成状态、剩余天数排序和截断；公告事件按上海当前日期过滤，避免行情水位落后时隐藏已发布公告。下修计数从发行/条款生效日与上市日中较晚者开始；成功下修从生效日重新计数；不下修锁定期结束后从交易日历归一化后的 `next_eligible_date` 重新计数；没有固定日期的“锁定至某次董事会召开日”保持锁定，等待后续公告。上交所转债若数学条件在交易日触发，且触发日及次一交易日没有已入库的修正/不修正响应，则按监管规则视为本次不修正，从次一交易日重新起算；该隐含日期写入计算诊断，后续出现正式公告时由正式事实优先。官方提议、股东大会和实施事件优先于数学计数；董事会提议超过 30 个自然日且无后续事件时回落到数学状态；已实施/未通过仅在最近 3 个交易日置顶，历史事件恢复数学状态。若条款声明“不得低于每股净资产”，包括“价格不低于最近一期经审计的每股净资产”等同义表达；用同一交易日正股收盘价/PB还原每股净资产；当前转股价不高于该底线时标记为 `floor_blocked`（净资产底线限制），保留数学进度但不进入“已满足待公告/接近触发”。接口同时提供 `remaining_days`（当前还差，按要求天数减已满足天数）和 `rolling_remaining_days`/`minimum_future_days`（滚动窗口最快还需）。深交所使用上市公司公告频道并分页，上交所/深交所来源失败时可从库内官方 PDF 重新解析；不下修解析器 `v7` 支持正文内嵌不下修决定、明确日期、债券到期日、首个交易日后重新起算和董事会事件锁定。迁移 090/091/092/093/094/095/096/100/101/102/103/104/105/106/107/109 负责建表、条款重解析、提议有效期、事件回填、滚动剩余天数、状态过期、解析质量门槛、净资产底线状态和视图重建；其中迁移 092 会先补齐 `minimum_future_days` 再重建视图，兼容旧生产库；条款解析器为 `terms-v3`。接口同时返回行情日、下修计算日和 `expected_trade_date`，公告游标错误或最新不下修解析积压时 `stale=true` 并返回 `quality` 诊断 | 触发价/正股收盘价/转股价/每股净资产：元；进度：已满足/要求交易日；剩余规模：亿元；状态：提议、股东大会待审/已通过、已实施、锁定、净资产底线限制、临近触发、跟踪、数据不完整；“数据不完整”仅表示真实缺少条款、交易日历、正股关联或公告解析 | 公告事实每日 07:40；行情完成后 08:15 与强赎、列表、估值串行计算；启动时检查公告游标、公告解析积压和下修派生结果水位，必要时先从库内官方 PDF 重新解析，再按游标重叠窗口补公告并只重算本地派生数据；交易日历、正股停牌、重新起算日、转股价变动和净资产底线值均写入诊断 |
...
| 可转债主链 | 收盘日次日 08:00 按上一交易日目标数据日同步，并补齐 `market.trade_calendar` 与正股行情缺口；08:15 执行估值；进程启动时校验新版强赎覆盖率 | 行情 → 周期 → 正股缺口补齐 → 强赎进度 → 上市列表派生指标 → 估值 → 预警；强赎必须达到全市场覆盖且公式版本一致，目标日空数据或覆盖率不足时保留上一快照并有限回看 |
| 可转债公告与下修监控 | 公告事实收盘日次日 07:40；首次全量按市场/年度窗口批量取，后续按 `ops.sync_cursors` 上次成功日期回扫 3 天重叠窗口；交易所主取失败或分页不完整才切巨潮，官方来源编号幂等去重，补发/更正覆盖原记录；启动时校验公告游标、解析积压、派生水位和下修计算逻辑版本，必要时仅重跑库内官方 PDF 解析并重算本地派生数据 | 公告事实 → 下修事件 → `reset-v2` 交易日进度；成功空结果不切备源；主备均失败时保留上一份有效结果、游标不前移；接口显式返回 `expected_trade_date`、`stale` 与公告质量诊断 |
...
| 可转债安全性 | 收盘日次日 06:30 | 按计划 `businessDate` 取上一交易日；先统一 `cb_basic` 生命周期日期并排除标准发行事实中“定向/私募”未上市债券，再计算覆盖率；目标日数据未形成完整分区时向前最多回看 5 个开市日，覆盖率低于 `min(预期活跃债券数, max(90% 预期数, 100))` 不发布；水位校验取全部快照中的最新数据日期；无快照时启动后补一次 |
...
任务通过 PostgreSQL advisory lock、计划实例租约和 `job_runs`/`ops.sync_cursors` 留痕。Web 与 worker 只能有一个负责调度。持久化执行器为每个计划实例建立唯一 `job_runs`，子进程不再写嵌套运行记录；`job_runs` 同时记录 `external_call_count`、`external_sources`、`datasets` 和 `error_type`。外部限流、额度耗尽和熔断恢复期间，计划实例进入 `waiting_external`，恢复后自动继续且不消耗业务重试次数。

任务定义统一声明 `externalSources`、`retryPolicy`、`catchupMode` 和 `dataDatePolicy`。可执行状态为到期的 `pending`、带未来 `next_attempt_at` 的 `failed`，以及外部来源恢复后到期的 `waiting_external`；`succeeded`、`degraded` 和未解除的 `blocked` 不进入业务执行队列。成功但水位落后只显示为 `degraded`，后续由调度同步入口只读数据库重验，不重新调用 Runner。外部任务先做数据库游标/业务表新鲜度门禁，`fresh` 结果的外部调用数为 0；多来源任务按所有必要来源的最慢水位确认，避免单一来源成功掩盖另一来源落后。任务结果返回的 `dataAsOf` 优先作为本次成功水位，数据库查询用于无结果时的交叉确认；无目标数据集的成功空跑明确标记为无需水位校验。手动补跑默认同样受门禁，只有管理员显式 `force=true` 才能强制重拉。`latest_only` 任务（打新日报、行情主档、个股分析、套利公告等）补跑窗口只生成一个最新计划实例，按日分区的收盘数据仍按缺失代码补齐。
...
外部请求预算计数写入 PostgreSQL `ops.external_call_budgets`，Token＋接口熔断写入 `ops.external_circuits`，数据集并发锁使用 PostgreSQL advisory lock；三者职责分离，由 Node 服务和打新日报/新股历史 Python 子进程共用。Python 子进程通过 `EXTERNAL_CALL_GUARD=1` 启用同一套保护，业务错误也保留 `api_name` 进入任务编排层；公告同步失败只记录 `last_error`，已有成功游标由数据库单调递增保护，不会因重叠窗口、旧进程收尾或并发写入回退。过期的 `job_runs` 租约由 Worker 自动回收，活动运行每分钟续租。
...
- 金融数据：`TUSHARE_TOKEN`、可选的 `TUSHARE_BACKUP_TOKEN`、`ANNOUNCEMENT_TUSHARE_FALLBACK`、腾讯行情 TTL、可转债同步并发和估值参数；外部请求保护可用 `<SOURCE>_PER_MINUTE_BUDGET`、`<SOURCE>_DAILY_BUDGET`（例如 `TUSHARE_PER_MINUTE_BUDGET`、`CNINFO_PER_MINUTE_BUDGET`、`TENCENT_PER_MINUTE_BUDGET`）调整预算，未配置时使用代码内安全默认值。后台外部 API 参数保存在 `platform_config.external_api_configs`，凭据加密且接口只返回掩码；IPO Python 子进程由 Node 注入后台当前的主备凭据与模式，不单独保存密钥。
