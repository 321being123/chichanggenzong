...
> 2026-09-10 版本 0.8.1.14：IPO 晚间 `enrichment` 以“下一交易日申购日或上市日”为优先目标，修正目标日期字符串标准化和上市日排序，保证发行阶段新股资料不会被较早历史缺口占用；本版本不新增表结构。
...
| 新股历史（`ipo_history`） | `ipo_history_sync.py` 是 `new_share` 唯一采集者：18:00 核心阶段增量 upsert 申购日、上市日、名称和代码，并优先补齐目标申购日的行业、行业 PE、主营业务和产品→下游业务暴露；19:30 先重新刷新 19:00 后的 `new_share` 发行公告，再重试发行资料、历史缺口和首日表现；下游暴露以 `business_exposure` JSONB 留存证据、权重和可信度 | `/api/ipo/history`、打新历史页面、打新日历/日报和预测校准（均读已入库事实）；历史接口同时返回 `history_stage`、`field_status`、`data_as_of` 及预测基础值/风口修正/可信度；新股历史中的“预测涨幅”在有预测和证券代码时链接至 `ipo-report.html?code=...` 打新详情，详情优先实时读取 `ipo_history` 和预测表，避免旧日报覆盖新补数据 | 启动追赶 + 60天重叠窗口；空响应/异常不覆盖旧值；发行阶段资料缺失不再标记为“未到期”，目标日行业/主营业务/业务赛道未补齐时质量门禁不通过；结果公告尚未给出的上市日期、中签率、超额认购倍数进入 `pending_not_due`，不伪造事实；预测在发行公告阶段即生成，缺少结果字段时记录 `model_imputed_fields`、`result_fields_pending`，并输出按样本外误差扩大的可能区间；质量不通过不发布；风口校准使用近365天非北交所新股的稳健中位数，相对同期市场基准并按样本数收缩，系数限制在 `0.80~1.50` | 股数：万股；发行价：元；募资/公开发行市值：亿元；中签率：%；业务暴露权重：0~1；风口可信度：0~1；质量状态：`complete/missing/pending_not_due`；分区质量：`passed` | 工作日18:00核心事实 + 19:30公告刷新/资料补全；18:05日报只消费当天已发布分区 |
...
| IPO事实同步 | 工作日 18:00 核心事实、19:30 公告刷新/资料补全；启动时检查水位 | 18:00 是 `new_share` 唯一采集入口，落库后先做数据库内行业拆分与业务暴露归一化，并优先补齐目标申购日的行业、行业 PE、主营业务和业务赛道，再校验目标日申购/上市集合、身份、上市日缺口并发布当天 `ipo_history` 分区；19:30 先刷新19:00后的发行公告，再同日重试发行资料并补首日表现；发行结果未公布字段明确显示待公告，预测同时显示发行公告版可能涨幅和区间；行业/赛道缺失显示待补全，不伪装成“其他赛道” |
