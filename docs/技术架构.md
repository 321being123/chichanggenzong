...
> 最后核对日期：2026-08-09。应用版本以根目录 `package.json` 的 `appVersion` 为准。
...
公告检索（港交所披露易 + 巨潮资讯）→ `ops.raw_records` 原始记录 → `event.documents` 标准化公告 → `event.arbitrage_cases` 套利案件（3类策略：A股现金选择权/要约收购/换股吸收合并、港股私有化、港股供股权）→ 腾讯批量行情计算折价率/年化收益 → 公开列表（仅审核通过且未结束的）→ 管理后台审核/重新解析/手动同步。

- **数据库**：迁移054，`event.arbitrage_cases`（主表，含 market/strategy_type CHECK约束、价格>0约束）+ `event.arbitrage_case_documents`（公告链关联表）；数据源代码 `hkex_announcements`/`cninfo_announcements`。
- **同步流水线**：首次1年同步（拆分为12+个月窗口）→ 增量同步（游标管理）→ 标题分类 → 标准化入库（事务内创建 instrument → document → case）。
- **PDF解析**：Python PyMuPDF 提取公告正文 + 正则解析证券代码/价格/比例/日期/要约人，输出 confidence + evidence。
- **公式版本**：v1.0（现金对价 calcCashArbitrage / 换股比例 calcSwapArbitgage / 供股权 rights issue，统一入口 calcArbitrage）。
...
> **单一任务注册清单（WORKER-01，2026-08-05）**：所有后台任务的「启动补漏 + 周期调度」统一收敛到 `server/scheduler.js` 的 `SCHEDULER_REGISTRY`，`server/app.js`（必须设 `DISABLE_SCHEDULER=1`，见 `deploy/portfolio-server.service`）与 `server/worker.js`（独立 systemd 进程，见 `deploy/portfolio-worker.service`）共用同一份，避免漏注册。注册表含 **9 个**周期任务（marketCloses / marketVolatilitySync / bondSafetyRefresh / stockAnalysisRefresh / convertibleBondRefresh / ipoCalendarRefresh / hkTradeRulesSync / holidaySyncMonthly / **arbitrageSync**）+ 4 个启动任务（holidaySync / backfillCloses / indexBaseline / hkTradeRulesStartup）。上表为该清单的时间口径。**生产已固定拆分：Web 关闭调度，全部后台任务由独立 worker 进程承担**（systemd 服务文件见 `deploy/`）。
