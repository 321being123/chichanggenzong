...
- 打新历史表「转债发行规模」保留3位小数显示（新增 `ipoNumFixed` 辅助函数）。
- 新股「中签率」显示改为固定保留3位小数（原为最多6位去尾零）。
- 打新日历补更新：赛斯发债（118073，7月17日申购）此前因日历快照生成早于公告未收录，已重新刷新日历数据补入。
- 打新日报/日历自动化：服务器新增每日 18:00 定时任务（root crontab），自动生成次日打新建议并更新打新日历，不再依赖手动触发。
