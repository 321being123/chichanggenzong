...
> 2026-09-22 可转债流通规模缺口治理（0.8.1.81，本地已实现，待标准部署）：`bond_listing_liquidity` 已登记为正式数据集；现有公告与生命周期任务的 17:30 槽位在抓取上市公告书后，按下一交易日上市债券集合核验覆盖。目标债券缺失时任务返回 `DATASET_INCOMPLETE`、保留缺口代码并进入现有重试，不再吞掉失败；早上的公告槽位只发布原有公告数据，避免重复抓取。`ipo_calendar_refresh` 增加对同业务日流通规模分区的质量依赖，未发布前不生成/覆盖日报。日报仍只读数据库，缺失时显示“流通规模待同步，本次预测未计入规模调整”，不显示内部错误码；不新增任务、表或数据源，生产回补与部署待本次标准流程执行。
