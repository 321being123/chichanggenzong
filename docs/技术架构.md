...
- 历史证券 ID 审计脚本 `server/scripts/auditInstrumentMerges.js` 负责候选和影响量；`server/scripts/migrateInstrumentMerges.js` 只迁移名称一致且唯一目标明确的组，按外键逐表更新，确定重复的 `core.company_instruments` 关系先去重，旧主档保留并标记 `status='merged'`，表达式/其他唯一冲突及人工核对项继续阻塞，不物理删除旧证券。统计外键影响时排除 `core.instrument_merge_candidates` 自身的审计记录，避免自引用放大数量。
