...
test('A/H并列现金退出价只提取A股人民币价格，并支持实施公告的行权价格', () => {
  ...
  const combined = parseSnippet([
    '证券代码：601995 证券简称：中金公司；证券代码：601198 证券简称：东兴证券；证券代码：601059 证券简称：信达证券。',
    '中金公司A股、H股异议股东收购请求权价格分别调整为34.80元/股、18.86港元/股。',
    '东兴证券异议股东收购请求权价格调整为13.04元/股。',
    '信达证券异议股东收购请求权价格调整为17.75元/股。',
  ].join(' '), '601198.SH');
  assert.equal(combined.cash_offer_price, 13.04);

  const combinedCinda = parseSnippet([
    '证券代码：601995 证券简称：中金公司；证券代码：601198 证券简称：东兴证券；证券代码：601059 证券简称：信达证券。',
    '中金公司A股、H股异议股东收购请求权价格分别调整为34.80元/股、18.86港元/股。',
    '东兴证券异议股东收购请求权价格调整为13.04元/股。',
    '信达证券异议股东收购请求权价格调整为17.75元/股。',
  ].join(' '), '601059.SH');
  assert.equal(combinedCinda.cash_offer_price, 17.75);
