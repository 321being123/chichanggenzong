...
| 可转债下修监控（`analytics.convertible_bond_revision_latest`） | 公告同步读取巨潮/上交所/深交所并写入公告事实、`event.convertible_bond_revision_events`；`calculateConvertibleBondRevisionStatus` 按 `reset-v2` 写入 `analytics.convertible_bond_trigger_daily` | `/api/bond-revision`、可转债“下修”子页 | 仅当前交易日有行情的交易所上市公开转债；退市、未上市、定向和私募债券不进入下修范围。公告首次全量，后续读取 `ops.sync_cursors` 的上次成功日期并回扫 3 个自然日；以官方公告来源编号去重，编号相同的补发/更正更新原记录；同步失败不推进游标并保留上次事实；页面只读库，不触发外部请求。迁移 090/091 建表与视图重建使用 PostgreSQL advisory lock，避免 Web/Worker 并发初始化 | 触发价/正股收盘价/转股价：元；进度：已满足/要求交易日；剩余规模：亿元；状态：提议、股东大会待审/已通过、已实施、锁定、临近触发、跟踪、数据不完整 | 公告事实每日 07:40；行情完成后 08:15 与强赎、列表、估值串行计算；交易日历、正股停牌和转股价变动均写入诊断 |
...
| 可转债公告与下修监控 | 公告事实工作日 07:40；首次全量，后续按 `ops.sync_cursors` 上次成功日期回扫 3 天重叠窗口；官方来源编号幂等去重，补发/更正覆盖原记录 | 公告事实 → 下修事件 → `reset-v2` 交易日进度；失败保留上一份有效结果，游标不前移 |
