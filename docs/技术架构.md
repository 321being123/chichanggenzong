...
> 2026-09-23 HKIPOx 本地实现与验收（0.8.1.83）：全量测试 142 通过、0 失败、0 跳过，知识门禁通过；短暂以非生产模式启动服务并仅连接隔离库 `portfolio_test`，迁移 162 成功登记匿名策略。`/health` 返回 `0.8.1.83`，HK IPO 日历/历史接口和详情页、JS 静态资源均返回成功；隔离库没有港股 IPO 样例，接口返回空列表，不代表生产数据。未运行真实任务采集、未运行生产迁移或部署。HKIPOx 仅在本地代码启用；HKEX 交易日历仍依赖 Tushare `hk_tradecal`。
