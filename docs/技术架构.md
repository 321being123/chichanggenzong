...
> 最后核对日期：2026-08-14。应用版本以根目录 `package.json` 的 `appVersion` 为准。
...
- 打新：可转债发行事件和档案由统一标准表保存，日报/页面只读数据库；独立新股历史任务继续负责 `ipo_history`。首次全量入库，之后保留60天重叠窗口；缺字段每日补偿，首个非涨停日表现单独回填；任务使用 PG 锁、`job_runs` 和 `ops.sync_cursors`，外部失败不推进水位。新债流通调整不再使用固定粗分档：按13档细分流通规模，以每只债的“实际首个非涨停日价格－未加流通调整的基础估值”除以上市前转股价值计算残差，再减同期全市场残差；近3个月权重70%，第4至6个月权重30%，窗口不足时使用近半年稳定项；同档不足4只时合并相邻档，仍不足则取最近规模6只，少于10只取中位数、否则两端各截尾10%，最终限制在-25至+35个百分点。预测日前的有效样本少于4只时调整为0，禁止使用未来结果。页面区分“亏损 / 收盘后更新 / 无历史预测 / 待补全”。
...
任务水位校验会先归一化业务日期；需要上一交易日数据的任务显式声明日期策略，静态或低频配置同步任务按执行结果判断，不强制要求当日水位。重新校验成功会自动恢复对应历史告警。套利 PDF 解析对超过大小限制的永久失败会直接记为耗尽，避免无效重试；其他解析失败仍按有限退避重试。
