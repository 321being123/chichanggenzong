...
可转债安全性财务缓存按证券增量刷新，单批最多 15 家（每家 3 个财务接口、最多 45 次调用），每批完成后跨到下一分钟再继续；旧缓存始终保留。这样即使 30 天 TTL 集中到期，也不会在数秒内耗尽主、备 Token 的分钟预算并连带阻塞后续任务。可转债安全性和行情主档遇到网络、证书或可疑空结果时按 15、60、240 分钟退避，空结果不再提前截断任务自有的四次尝试。休市日月度自愈由 Worker 每日检查一次是否需要更新，禁止使用超过 Node 定时器上限的 30 天 `setInterval`。后台“待处理告警”只包含仍需介入的故障；已发送的恢复通知和主备切换通知保留历史记录，但不计入待处理数量。
