...
> 2026-09-21 港股汇率实时性修复（0.8.1.76，本地已实现）：生产出口访问 Yahoo Finance 图表接口会返回 HTTP 429，导致交易时段请求回退每日汇率；盘中改读可实际访问的 [Yahoo Finance Japan HKDCNY=X 行情页](https://finance.yahoo.co.jp/quote/HKDCNY%3DX)，按 5 分钟数据库快照和 Guard 规则复用/刷新。港股持仓首次加载也按交易时段请求实时汇率，前端自动刷新改为 5 分钟，并统一按上海时区判断港股交易时段；实时源失败仍回退最近有效值，不改历史汇率。
