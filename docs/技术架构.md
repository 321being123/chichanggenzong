...
- 打新：日报任务只生成日历、报告和当日分析；独立新股历史任务负责 `ipo_history` 名单、发行详情、质量状态和上市首日表现。首次无游标回看两年，之后保留60天重叠窗口；缺字段每日补偿，首日涨幅最多补抓3个工作日；任务使用 PG 锁、`job_runs` 和 `ops.sync_cursors`，外部失败不推进水位。API 只读数据库。页面区分“亏损 / 收盘后更新 / 无历史预测 / 待补全”。
...
| 数据源 | 用途 | 失败策略 |
|---|---|---|
| Tushare Replay | 统一转发 A 股、财报、指数、可转债、交易日历、宏观数据及 `new_share` 新股发行清单；统一入口 `server/services/tushareReplay.js` / `ipo-report/_common.py`，GET + `X-API-Key` | 页面优先读库；定时任务失败或可疑空结果保留旧数据并记录失败；旧官方 Tushare POST 链路已移除 |
...
- 金融数据：`TUSHARE_REPLAY_API_KEY`、可选 `TUSHARE_REPLAY_BASE_URL`、Replay 超时/重试、腾讯行情 TTL、可转债同步并发和估值参数。
