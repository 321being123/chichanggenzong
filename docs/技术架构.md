...
> 2026-09-19 版本 0.8.1.70：`dependency_blocked` 槽位告警允许进入既有的中立恢复证据层，只有原槽位成功且运行、阶段、严格分区和水位证据完整时才能收敛；`data_quality` 槽位告警仍被拒绝，必须使用对应数据集分区证据。生产三日可转债行情补跑已全部成功，代码待标准发布后关闭两条下游阻塞告警。
