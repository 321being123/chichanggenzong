...
| 港股 IPO 动态申购信号（`hk_ipo_subscription_signals`） | `server/services/hkIpoMarketSignals.js` 将第三方时点信号写入 `analytics.hk_ipo_market_snapshots`；迁移 161 增加来源观测时间、孖展金额/倍数、信号类型、来源记录哈希和质量状态；同一来源内容/上游时刻幂等，数值或上游时刻变化保留新快照 | `/api/ipo/calendar`、`/api/ipo/history`、`/api/ipo/report/code` 只读 `quality_status='valid'`，按来源阈值派生 `fresh/stale`，不把陈旧状态回写事实；`datasetPartitionRegistry` 按 `HK + signal_type=subscription + data_date` 登记分区 | 动态源失败只返回降级诊断并保留历史有效快照，不阻断 `hk_ipo_facts`；盘中槽位仍须通过授权、双环境结构探针、5 日稳定性和真实正样本门禁后才能开启 | 金额：HKD；倍数/价格/涨跌幅：标准数值；时间：来源观测时间与首次采集时间 | P0 已在本地隔离库实现；来源准入和附加盘中槽位未开启 |
