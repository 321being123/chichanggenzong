...
> 最后核对日期：2026-08-19。应用版本以根目录 `package.json` 的 `appVersion` 为准。
...
- 可转债详情历史：`refreshConvertibleBondAnalysis` 以主档 `list_date` 为公告起点；主档缺少上市日时回退到 `value_date`，再由巨潮/交易所历史公告解析转股价调整和不下修记录。利息明细优先使用 `cb_rate` 或募集说明书解析结果，接口不可用时从已落库的 `rate_clause` 生成年度利率明细；PDF 解析器按 `IPO_PYTHON_PATH`、项目虚拟环境、`ipo-report/venv` 顺序选择 Python；详情页只读取 `analytics.analysis_snapshots` 和标准历史表，不调用受限的 `cb_price_chg`。
