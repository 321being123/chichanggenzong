...
(async () => {
  ...
  try {
    ...
    // B. 模拟"已有旧表"：删除 034 加列、035 视图与函数、并清除迁移记录 → 重跑应幂等恢复
    ...
    await db.pool.query(
      `DELETE FROM schema_migrations WHERE version IN ('034_bond_profile_list_date','035_bond_unified_stk_fallback')`
    );

    await migrations.runMigrations();
    const appliedB = await db.pool.query(
      `SELECT version FROM schema_migrations WHERE version IN ('034_bond_profile_list_date','035_bond_unified_stk_fallback')`
    );
    check('旧表升级场景：重跑后 034/035 重新登记', () => {
      assert.strictEqual(appliedB.rows.length, 2);
    });
