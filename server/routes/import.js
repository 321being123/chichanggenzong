...
const { visionUploadTokens, TOKEN_TTL, mobileUploadHtml } = require('../services/vision');
...
router.post('/api/vision-upload/:token', (req, res) => {
  ...
  // 仅接受图片 MIME；限制解码后大小（上限 10MB）
  const m = /^data:(image\/[a-zA-Z0-9.+-]+);base64,(.*)$/.exec(image);
  if (!m) return res.status(400).json({ ok: false, error: '仅支持图片文件' });
  const size = Math.floor(m[2].length * 3 / 4);
  if (size > 10 * 1024 * 1024) return res.status(413).json({ ok: false, error: '图片过大（上限 10MB）' });
  entry.image = image;
...
router.get('/api/vision-check/:token', requireLogin, (req, res) => {
  ...
  // 一次性消费：取走图片后即销毁 token，防止重复拉取/重放
  visionUploadTokens.delete(token);
