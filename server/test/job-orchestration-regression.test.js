...
assert(/expectedDataDate\('bond_safety_refresh', businessDate\)/.test(jobRunners)
  && /runBondSafetyRefresh\(reason, \{ targetTradeDate, readOnly: true \}\)/.test(jobRunners), '可转债安全评分必须按计划业务日期传入目标交易日并只读标准层');
...
assert(/const JOB_LABELS/.test(adminUi) && /ipo_history_sync: 'IPO事实同步'/.test(adminUi) && /arbitrage_reparse: '套利公告重新解析'/.test(adminUi), '所有后台任务必须使用中文名称');
...
assert(/const typed = message\.match\(\/\\\[\(BUDGET_WAIT\|RATE_LIMIT\|QUOTA_EXHAUSTED\|CIRCUIT_OPEN\)/.test(arbitrageParser)
  && /error\.code = typed\[1\]\.toUpperCase\(\)/.test(arbitrageParser), 'Python 解析器的来源限速错误必须透传给统一任务编排');
