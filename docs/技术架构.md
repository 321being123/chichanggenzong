...
> 2026-09-16 本地修复（待发布）：可转债上市公告书、前十持有人和流通规模补全先按正股代码/上市日期查询上交所或深交所官方公告 PDF；交易所无资料、下载失败或解析失败时，再以 CNINFO 同类公告兜底，来源按实际结果留痕。
> 2026-09-16 本地修复（待发布）：IPO 招股书、港股配发/取消上市资料、可转债上市流通/发行结果资料接入 Node/Python 共用的统一持久化 PDF 缓存；默认 30 天、总量 5 GiB，命中缓存不重复下载，不新增数据库结构。
> 2026-09-16 本地修复（待发布）：A 股 IPO 资料补全新增交易所发行公告解析，公告明确披露的所属行业和 T-3 静态行业市盈率优先于 Tushare；主营业务继续取交易所招股说明书，并由正文归类业务赛道。上交所 PDF 使用官方可下载镜像，来源 URL、标题、日期、正文哈希和解析器版本随补全结果留痕；不新增数据库结构。
...
3. **来源优先级**：上交所、深交所、北交所、港交所等官方交易所公告为事实主源；巨潮资讯只在具体任务契约允许时作为资料备源或兜底，Tushare 仅作为明确登记的末级备源。官方主链禁止回退时，不能在业务代码中自行切换巨潮。
...
当前已统一的事实是：可转债公告、生命周期、下修/转股价和强赎事件共享官方公告批次；套利 A 股公告使用上交所/深交所官方接口，港股使用港交所；IPO 招股书和上市流通资料仍按各自契约执行交易所优先及允许的巨潮兜底，但 PDF 原文已通过统一持久化缓存跨 Node/Python 复用，消除同一 URL 的重复下载。缓存命中先本地解析，缓存缺失、过期或损坏时才重新请求；解析失败保留原文供后续重解析，不以失败覆盖已存在事实。

### 8.2.2 后台任务、质量和熔断不变量（P0-2～P0-7）

- **接口限制与熔断**：只登记已核验的具体接口限制；未知上限保持 `NULL`。接口限流/权限拒绝只影响对应接口，内部等待、槽位批次止损和真实上游 429/额度异常分别处理；只有最后一种才开启对应接口熔断。熔断状态统一写入 `ops.external_circuits`，恢复时间、探测结果和实际作用域必须可查，Node、Python 与脚本共用同一策略。
...
- **告警恢复**：告警按槽位、任务、数据集分区或来源＋接口＋凭据绑定作用域。数据质量告警必须由对应分区和质量证据关闭，来源告警还必须有熔断关闭和新的成功时间；空查询、手工改状态和单次普通任务成功都不能关闭告警，恢复通知按告警键去重。

当前代码已具备来源级策略表、接口级熔断、计划槽位/数据集分区和恢复证据等基础能力。迁移 156 已清除来源/凭据级内部数字保护线，并以数据库约束禁止通配策略再次写入内部限额；只有具体 `api_name` 的已核验限制可以生效，接口限制不会汇总成跨接口凭据总量。可转债上市流通、港股 IPO 招股书/配发结果和港股日线补全已取消默认固定条数，默认处理全部待补对象，显式调试上限会以部分完成记录，不得冒充成功。通用任务治理静态门禁已接入 `check:knowledge`，分析页面无快照或过期时只提示用户手动刷新；IPO 文档持久化缓存已补齐并由 Node/Python 共用，显式批次续跑的通用运行态验收仍需补齐，未完成前不得宣称相关链路全量合规。
...
本轮已落地方案第 17 节身份基础设施和首批事实接口：迁移 140/141 为 `ipo_history` 增加 `market_code`、`ipo_status`、`ipo_status_at`、`instrument_id` 外键并回填 CN；迁移 142 增加港股阶段事实字段、事件时刻和推荐快照容器；迁移 143 让证券数字代码唯一性包含市场维度。`ipo_history_sync.py` 新写入先经统一身份模块创建/复用 `core.instruments`，上市状态不会把已上市主档降级，A 股查询显式限定 `market_code='CN'`。新建 `hkexIpo` 解析器和只读探针，白名单覆盖 `www1.hkexnews.hk`、`www2.hkexnews.hk` 及官方新上市站点；新增官方历史报表回填入口，原始报表保留在 `ops.raw_records`；港股任务、HKEX 日历、HK 分区和 `/api/ipo` 的 CN/HK 过滤已接入。迁移 156 后不再保留来源级或凭据级内部分钟/日限额，只有已核验的具体接口策略可以设置限制，未知值保持空。双环境探针、港股日线覆盖、历史回测审计链已完成接入，但正式建议门禁仍按下方 P0 状态阻断。
...
| 上交所、深交所公告接口 | 可转债公告主取；由统一公告采集层按市场/年度窗口批量拉取，保留来源编号、证券代码和官方 PDF 地址 | 成功空结果不切备源；失败或分页不完整保留 `complete=false`、不推进游标并等待重试，不自动切换巨潮；结果先写 `ops.raw_records`/`event.documents` 再投影到业务事实，避免逐债重复请求 |
...
外部请求预算计数写入 PostgreSQL `ops.external_call_budgets`，Token＋接口熔断写入 `ops.external_circuits`，数据集并发锁使用 PostgreSQL advisory lock；三者职责分离，由 Node 服务、Python 子进程和历史 PDF 解析脚本共用。Python Guard 默认开启，只有开发/测试显式 `EXTERNAL_CALL_GUARD=0` 才可关闭，生产环境即使设置 0 也强制开启；两端统一调用 `ops.reserve_external_call(source, api_name, credential_profile, credential_fingerprint)`，在 `external_budget:{source}:{credential_fingerprint}` 锁下按来源＋接口＋凭据原子计数，并使用接口并发槽和最小间隔运行态。预算日键始终取请求实际发生的上海自然日，不能被补跑任务的 `JOB_BUSINESS_DATE` 污染。迁移 156 后的规则是：来源级 `api_name='*'` 不设置内部分钟/日限额，数据库约束禁止重新写入；只有已核验的具体接口可以设置官方或内部限制，未知值保持 `NULL`，接口计数不会汇总成跨接口凭据总量。当前实际使用的腾讯行情、上交所和深交所公告来源不设置内部分钟/日预算，仅保留并发去重和真实上游异常处理；其他来源也必须遵循同一接口级策略，不能依赖来源级兜底数字。内部接口预算到限使用 `BUDGET_WAIT` 等待，不切备用 Token、不写 Token 熔断；真实上游 429/额度错误才按实际接口作用域熔断并保留恢复时间。`/api/hkrate` 和持仓对比在抓取等待/失败时读取最近有效汇率，不能因为一次外部请求失败阻断页面或估值。来源告警恢复必须在熔断全部关闭且存在新的 `last_success_at` 证据后由 Node 统一核对，手工关闭熔断不直接关闭告警。预算策略、官方权限、重试/超时/空结果规则通过后台“外部 API 主备”的接口策略面板维护；Token 只保存加密值，策略表只保存角色和不可逆指纹。交易所 PDF 地址按域名归属来源，`disc.static.szse.cn` 统一计入 `szse`，不落入未配置的通用来源。0.7.0.18 已完成原有额度整改的生产验收闭环；汇率误熔断修复已纳入 0.7.0.28，生产提交 `12fe5b5` 已验收，详情见 `INC-0006`。

接口限额策略由迁移 127、128、145、156 统一落地到 `ops.source_endpoint_policies`：按 `source_id`、`api_name`、`credential_profile` 保存官方最低积分、权限模式/状态、官方分钟/日限额（未知允许 `NULL`）、内部分钟/日限额、并发、最小间隔、行数上限、超时、重试/空结果策略、启用状态、最近核验时间和备注；仅保存不可逆凭据指纹，不保存原 Token。来源级 `api_name='*'` 仅承载并发、间隔、权限等通用配置，不得设置内部分钟/日限额；`ops.source_endpoint_runtime` 保存接口＋凭据的最小间隔状态，`ops.external_call_budgets` 的主键已扩展为“来源＋接口＋凭据指纹＋窗口”，Node、Python 和历史解析子进程均通过 `ops.reserve_external_call` 原子执行接口级计数。接口限制达到保护线进入等待，不能汇总成跨接口凭据总量；真实上游 429/额度错误才记录实际接口作用域熔断及恢复时间。上交所、深交所仅保留并发去重和真实上游异常处理；后台接口策略面板支持按主/备凭据分别调整已核验规则，Token 轮换时自动更新指纹并重置待核验状态。
