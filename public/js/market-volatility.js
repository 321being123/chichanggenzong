...
function mvRenderChart() {
  ...
  if(window.ChartInteraction)ChartInteraction.watch(root,mvRenderChart);
  var W=window.ChartInteraction?ChartInteraction.width(root,1000):1000,H=320,L=55,R=20,T=18,B=36,ph=H-T-B; function x(i){return L+i*(W-L-R)/Math.max(data.length-1,1)} function y(v){return T+(max-v)*ph/(max-min)};
  ...
  ChartInteraction.bind(chart,root.querySelector('.mv-tooltip'),function(e){mvShowTooltip(e,chart,data,L,R,W);},function(){root.querySelector('.mv-tooltip').hidden=true;});
}
