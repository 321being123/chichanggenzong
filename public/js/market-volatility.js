...
function mvRender() {
  ...
  document.getElementById('mv-recommended').textContent = recommended == null ? '--' : recommended + '%'; document.getElementById('mv-actual').textContent = mvPct(o.actualPosition); var actualLabel=document.getElementById('mv-actual').previousElementSibling; if(actualLabel)actualLabel.textContent=username&&mvAccount()?'实际股票仓位（'+mvAccount()+'）':'实际股票仓位';
