...
check('前端含上市转债子页、脚本和只读接口', () => {
  ...
  assert.ok(sharedTable.includes('biz-table-head-label') && sharedTable.includes('normalizeHeaders') && sharedTable.includes('splitLongHeaderLabel') && sharedTable.includes('Math.ceil(chars.length / 2)'), '共享表格必须统一按中点处理长标题换行');
