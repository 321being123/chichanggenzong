...
3. **熔断处理**：预算、数据集锁和熔断分开记录；交易所、巨潮、Tushare 分来源隔离。HTTP 429、5xx、网络异常和分页不完整进入备源；“请求成功但 0 条”视为正常空结果，不触发备源或熔断。深交所批量接口可能实际返回少于请求的页大小，必须按 `announceCount` 与累计行数判断是否完整，不能把短页直接当作结束；达到分页上限时保留 `complete=false` 并进入备源。主备均失败时不推进 `ops.sync_cursors`，保留已有公告和解析结果，下一次按 3 天重叠窗口补偿。非 Tushare 的来源级熔断提示显示“来源已熔断”，避免误报成 Token 熔断。
