...
保荐人分组解析器已兼容跨页参与方标题、`Sponsor-OCs` 多角色表头、单独 `Sponsor` 角色、单数 `Intermediary` 和 `Hong Kong SAR` 地址边界；官方英文招股书定向重解析 run 608—609、615 后分组覆盖达到 165/165，集中度分组覆盖 100%，角色标题、地址和短词脏值反查为 0。最新正式回测为 `backtest_id=73`：候选/可回测/推荐档 165/165/162，首日/第五交易日覆盖 100%，分组 108 组、最大组占比 10.49%，硬否决 0。剩余门禁阻塞是实际收益指标（破发后验均值 0.260736、上限 0.305644、扣费净盈利率下限 0.217707、每手扣费净收益中位数 -85.22 港元）、双环境稳定工作日 2/5 和 walk-forward 2/3；不得用估算、历史日期回填或放宽阈值替代。当前 165 条样本的 `issue_pe`、`industry_pe`、募资额、总股本、公开认购倍数均无值，评分特征仍不完整；补齐官方历史特征后必须新建模型版本并重新回测。全量测试 122/122 通过，知识治理检查和差异空白检查通过；正式建议仍只读事实，代码已完成生产部署，未执行生产历史数据同步。
