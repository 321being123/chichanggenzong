...
> 最后核对日期：2026-08-15。应用版本以根目录 `package.json` 的 `appVersion` 为准。
...
| `ops` | 数据源、采集批次、原始响应、同步游标、质量问题、外部请求预算 | `data_sources`、`ingestion_runs`、`raw_records`、`sync_cursors`、`data_quality_issues`、`external_call_budgets`（迁移076） |
...
- **锚点续算公式**：最新导入日保存券商现金 `B_cash`、券商持仓合计 `B_pos`、当时系统持仓估值 `S_pos0`。之后任意时点：
  `effectiveCash = B_cash + 导入日之后现金流 + 导入日之后交易结算`；
  `effectivePosition = B_pos + (当前系统持仓估值 - S_pos0)`；
  `effectiveTotalAsset = effectiveCash + effectivePosition`。
  因此导入日总资产/现金精确采用券商口径，之后只叠加本系统行情和资金增量；再次导入会以最新券商快照重置锚点。
- **汇率差异说明**：券商总资产可能使用券商汇率，本系统逐证券明细使用系统汇率。没有券商逐证券估值/汇率明细时，不能伪造单证券券商市值；页面总资产显示权威锚点，持仓明细明确为系统参考估值，导入口径差不再显示为“历史快照校准差额”。
- **历史锚点输入**：`S_pos0` 必须使用导入日 `daily_prices` 与该日系统汇率；缺少任一持仓价格或港股汇率时拒绝建立精确历史锚点，不回退到今天价格。
- **统一归因**：`server/services/navAttribution.js` 按统一区间计算价格、汇率、账本影响，`core-tables.js` 只展示后端结果；完整数据下归因闭合，缺数据时显示缺失状态，不制造“历史快照校准差额”。
...
任务定义统一声明 `externalSources`、`retryPolicy`、`catchupMode` 和 `dataDatePolicy`。可执行状态仅为到期的 `pending` 或带未来 `next_attempt_at` 的 `failed`；`succeeded`、`degraded` 和未解除的 `blocked` 不进入业务执行队列。成功但水位落后只显示为 `degraded`，后续由调度同步入口只读数据库重验，不重新调用 Runner。外部任务先做数据库游标/业务表新鲜度门禁，`fresh` 结果的外部调用数为 0；手动补跑默认同样受门禁，只有管理员显式 `force=true` 才能强制重拉。`latest_only` 任务（打新日报、行情主档、个股分析、套利公告等）补跑窗口只生成一个最新计划实例，按日分区的收盘数据仍按缺失代码补齐。

Tushare 请求现在保留上游 HTTP/业务错误码并分类为网络、空数据、限额、权限或参数错误；缺少 `TUSHARE_TOKEN`、权限码（如 2002/40101）和参数错误均明确失败且不自动重试；429/配额错误当天熔断，网络错误使用外部任务退避（15/60 分钟）。收盘任务把单次运行的内层请求和外层重试纳入同一个请求预算。

外部请求预算、每日熔断和“数据源 + 数据集 + 业务日期”并发锁统一写入 PostgreSQL `ops.external_call_budgets`，由 Node 服务和打新日报/新股历史 Python 子进程共用；Python 子进程通过 `EXTERNAL_CALL_GUARD=1` 启用同一套保护，首次完整请求入库，后续按游标和新鲜度门禁增量请求。过期的 `job_runs` 租约由 Worker 自动回收，活动运行每分钟续租。
...
- 金融数据：`TUSHARE_TOKEN`、腾讯行情 TTL、可转债同步并发和估值参数；外部请求保护可用 `<SOURCE>_PER_MINUTE_BUDGET`、`<SOURCE>_DAILY_BUDGET`（例如 `TUSHARE_PER_MINUTE_BUDGET`、`TUSHARE_DAILY_BUDGET`、`TENCENT_PER_MINUTE_BUDGET`）调整预算，未配置时使用代码内安全默认值。Python 子进程继承数据库连接环境变量和 `JOB_BUSINESS_DATE`，不单独保存密钥。
