...
//   - 双读核对：统一读取者返回的证券集合 ≡ 权威源 bond_history（按六位 security_code）。
...
(async () => {
  ...
  try {
    ...
    // 1) 双读核对：bond_history（打新日报权威源）每只证券都必须被统一读取者命中（无遗漏）
    //    注：bond_unified 主档比 bond_history 更广（含未进历史的在市转债），故用「覆盖核对」而非集合相等。
    const list = await bondSvc.getBondHistoryList(100000);
    const { rows: histRows } = await pool.query(
      `SELECT security_code FROM bond_history
    ...
    check('双读核对：bond_history 证券均被统一读取者覆盖（无遗漏）', () => {
      const missing = oldCodes.filter(c => !newCodes.includes(c));
      assert.strictEqual(missing.length, 0,
        '以下 bond_history 证券未被统一读取者覆盖: ' + missing.slice(0, 10).join(','));
    });
