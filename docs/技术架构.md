...
> 最后核对日期：2026-09-23。应用版本以根目录 `package.json` 的 `appVersion` 为准。
> 2026-09-23 A股 IPO 与日报依赖修复（0.8.1.84 已生产验收）：发行风险特别公告解析保留行业 PE、口径日期与公告来源；空发行 PE 不再仅凭上市日期推断亏损；可转债流通规模 LIKE 条件改为参数绑定。`301660` 线上确认为 C39 行业 PE `73.18`（基准日 `2026-09-18`），`73.66` 未获同口径公告证实；日报依赖链已恢复，19:35 enrichment 尚余 18 项历史字段缺口。
> 2026-09-23 港股 IPO 无 Token 来源设计（实施前核查记录）：现有港交所主板官方入口从本地做只读探针可解析当日发行项目；当时生产 `0.8.1.82` 的日历接口已展示五只正在招股项目，历史接口中这五只的申购期倍数和孖展信号均为空。本地旧库不能代表线上状态。原方案评估在现有 `hk_trade_calendar_sync` 和 `hkDailyCoverage` 中分别替换 Tushare `hk_tradecal`/`hk_daily` 的 IPO 依赖；具体调查证据见《港股IPO数据源与实时申购信号整改研发方案_20260920.md》第 15 节。
> 2026-09-23 公开网页只读补查记录：HKIPOx 当日 IPO 倍数直接出现在 HTML，匿名 GET 本机/生产均为 200；GAPWAY 页面有倍数和认购额，但生产 GET 403；SL886 本机 GET 403。来源条款、精确口径及来源观测时间仍需核验，不能把参考倍数解释为孖展或最终超购。
> 2026-09-23 HKIPOx 生产验收（0.8.1.83）：全量测试 142 通过、0 失败、0 跳过，知识门禁通过；提交 `14cfc9e` 已按标准流程部署，迁移 162 登记匿名策略成功，加密备份 `/var/backups/portfolio/portfolio_20260923_171621.sql.gz.gpg`。17:21 只读核验 Web、Worker、健康检查定时器均 `active/enabled`，`/health=ok/0.8.1.83`，`/ready` 数据库与 Redis 正常；港股日历返回 5 只正在申购新股，历史接口、IPO 页面与 HKIPOx 来源标签正常。首次 19:40 采集尚未执行，生产 HKIPOx 快照数为 0，真实信号待首轮任务后验收。HKIPOx 页面使用条款尚未查明；港交所交易日历仍依赖 Tushare `hk_tradecal`，无 Token 全链路验收未完成。
> 2026-09-23 后台告警与阶段契约治理（本地验证通过）：IPO 任务按 `prediction_ready/core/enrichment` 声明发布数据集和已发布分区依赖；前置与补全阶段只按自身进度完成，19:35 不重发核心分区。分区恢复同时校验注册表 `scope_key`。数据集告警只从当前任务声明且本轮尝试的数据集生成，`not_run` 不告警；重复成功限定同一计划槽位。港股第三方动态源未准入时安全跳过，不触发外部请求，动态信号仍显示不可验证。全量测试 142 通过、0 失败、0 跳过，知识门禁通过；生产部署与历史告警处理待完成。
> 2026-09-22 可转债流通规模缺口治理（0.8.1.81，已生产验收）：`bond_listing_liquidity` 已登记为正式数据集；现有公告与生命周期任务的 17:30 槽位在抓取上市公告书后，按下一交易日上市债券集合核验覆盖。目标债券缺失时任务返回 `DATASET_INCOMPLETE`、保留缺口代码并进入现有重试，不再吞掉失败；早上的公告槽位只发布原有公告数据，避免重复抓取。`ipo_calendar_refresh` 增加对同业务日流通规模分区的质量依赖，未发布前不生成/覆盖日报。日报仍只读数据库，缺失时显示“流通规模待同步，本次预测未计入规模调整”，不显示内部错误码；不新增任务、表或数据源。提交 `83502c8` 已完成标准生产部署，线上 `/health=ok/0.8.1.81`、`/ready`、Web/Worker/健康检查定时器均通过；生产备份为 `/var/backups/portfolio/portfolio_20260922_201431.sql.gz.gpg`。随后定向回补 `113710` 成功，流通规模 `5.8914` 亿元、限售规模 `4.3439` 亿元，来源上交所；数据集分区和 2026-09-23 日报已重新生成，最终展示约 `165` 元。
> 2026-09-22 IPO资料诊断展示补丁（0.8.1.80，已生产发布）：在 0.8.1.79 的来源诊断基础上，报告按字段空值和 `field_states` 共同展示来源暂不可用/文档未找到等原因；修正发行公告下载失败与解析失败的状态标记。提交 `f5afc62` 已完成标准生产部署，4 只受影响新股通过同一 `ipo_history_sync` Runner 定向补跑，失败 0；仍不新增任务、表或数据源。
> 2026-09-22 IPO资料诊断修复（0.8.1.79，已生产发布）：A股 IPO 主营业务资料继续按交易所官方招股说明书/招股意向书→巨潮→Tushare 顺序取数，新增“业务聚焦于/核心业务为”句式；仅缺行业 PE 时先读取交易所发行公告，再回退本地行业样本中位数。资料缺口在 `data_quality_status.field_states` 和 `source_payload.historical_enrichment` 中区分 `document_not_found`、`document_unavailable`、`document_parse_failed`、`document_field_absent`、`source_unavailable` 与 `value`，报告只读这些已落库诊断，不在页面请求时联网；定向补跑仍走 `ipo_history_sync` 统一 Runner，不新增任务、表或数据源。
> 2026-09-22 IPO 上市前预测时序修复（0.8.1.78，已生产发布）：`ipo_history_sync` 在 16:30 只针对下一交易日申购或上市且存在预测资料缺口的新股执行 `prediction_ready` 检查；先复用 `core.instruments.raw_data` 和 `market.daily_valuations`，只对仍缺失的字段定向补齐；打新日报仍按原计划 18:05 发布，18:00/19:30/19:35 原有事实同步、复核与全量补全继续保留。19:35 排除同日 16:30 已尝试的目标代码，避免重复请求。提交 `ed0419e` 已完成标准生产部署，线上版本、就绪状态、Web/Worker/健康检查定时器和相关 IPO 接口均验收通过。
> 2026-09-22 巨潮权限恢复探针修复（0.8.1.77 发布范围）：生产只读核查显示 `ops.external_circuits` 当前无打开的巨潮临时熔断，但 `document`、`topSearch` 权限策略仍停在历史 `permission_denied + enabled=false`，且当天没有新的巨潮采集记录；原机制只在更换凭据时重测，无法知道接口当天是否恢复。现复用 `ops.source_endpoint_runtime`，由每日套利同步在解析重试阶段为每个被停用接口最多执行一次轻量权限探针；文档地址使用 HEAD，查询接口使用最小 POST，403 只顺延下一次探针，成功才恢复真实接口策略，不参与公告发现、不新增表结构。解析器将外部权限/限流失败与 PDF 内容解析失败分开计数，前者不消耗 3 次文档解析额度，恢复后可继续重试。
> 2026-09-21 港股汇率实时性修复（0.8.1.76，已生产发布）：生产出口访问 Yahoo Finance 图表接口会返回 HTTP 429，导致交易时段请求回退每日汇率；盘中改读可实际访问的 [Yahoo Finance Japan HKDCNY=X 行情页](https://finance.yahoo.co.jp/quote/HKDCNY%3DX)，按 5 分钟数据库快照和 Guard 规则复用/刷新。港股持仓首次加载也按交易时段请求实时汇率，前端自动刷新改为 5 分钟，并统一按上海时区判断港股交易时段；实时源失败仍回退最近有效值，不改历史汇率。提交 `b91b6ab` 已完成标准生产部署，线上行情页 HTTP 200 且成功解析报价，`/health`、`/ready`、Web/Worker/健康检查定时器均验收通过。
> 2026-09-21 套利公告历史来源接管修复（本地已验证，未部署）：A 股公告发现仍按上交所/深交所官方接口优先；解析重试时，若同一事件链已存在同证券、同公告日、同规范化标题的交易所 PDF，则将历史 CNINFO 文档标记为 `superseded`，仅保留审计证据，不再重复解析或阻断案件质量。补充历史 CNINFO 文档没有 `announcement_merge_key` 时的代码/日期/标题匹配，确保交易所公告能接回原事件链；不新增数据库结构、不删除原始文档。当前线上槽位 `916128` 的 4 条待处理历史巨潮文档暂未发现对应 SSE/SZSE 原文，因此仍保留人工告警，不以无依据的交易所文档替代。
> 2026-09-20 港股 IPO 整改本地复核：`analytics.hk_ipo_market_snapshots` 已支持多时点哈希快照；动态信号失败不发布 `hk_ipo_subscription_signals` 本次分区，核心事实仍可独立完成，并由编排层分别生成数据集降级告警和精确 `source:api_name` 告警。`/api/ipo/history?type=hk_stock` 与详情接口分别返回申购信号、预计孖展信号和只读已落库盘中变化列表，页面同步分栏展示；来源时间、本地采集时间和 stale 状态沿用快照查询口径。用户已确认当前没有第三方动态源授权，系统保持影子/降级，不启用盘中附加槽位；生产迁移仍未执行。
> 2026-09-20 主备接口错误分类复核：Tushare 主接口已获得明确上游错误后，若备用接口仅因本地策略未配置或被禁用而无法尝试，保留主接口的限流/权限错误，不用备用策略错误覆盖真实原因；备用接口策略可用时仍按既有主备切换。
> 2026-09-19 本地治理门禁增量（0.8.1.72）：任务定义在默认值合并前提供原始配置审计；执行与恢复路径对未知 `jobCode` 失败关闭；数据集依赖统一消费 `partitionDatePolicy`（`business_date`、`previous_trading_day`、`expected_data_date`）；告警恢复使用 `ALERT_EVIDENCE_POLICIES` 注册表，具体 `source:api_name` 只核对精确接口，不能由 `source:*` 通配证据替代；规则追踪矩阵和真实测试库分区端到端回归已补齐。生产发布仍需单独授权。
...
2026-09-17 后，公告批量采集对单个“分页不完整/临时请求失败”执行一次有界重试；第二次仍不完整才切换 CNINFO 或保留游标，避免一次性源抖动造成公告缺口。Tushare 客户端仅对调用方明确声明的空数据启用主备切换，默认仍保留空结果语义；主接口已有明确上游错误而备用策略未配置/被禁用时，最终仍保留主接口错误分类，避免误报为备用策略缺失。
...
- **分阶段数据集契约**：同一任务存在多个业务阶段时，每个模式必须显式声明本阶段发布的数据集、依赖的已发布分区和阶段完成条件。无发布数据集的阶段仍须以自身剩余对象/失败对象判定完成；补全阶段只能校验依赖分区，不能覆盖前置核心分区。
...
- **任务契约门禁**：`JOB_DEFINITION_SOURCE` 与 `JOB_CONTRACTS` 在应用 `DEFAULT_JOB_OPTIONS` 前由 `validateJobDefinitionSources()` 审计，检查任务键重复/孤儿、字段冲突、日期策略枚举、数据集硬依赖子集和注册表命中；阶段一报告存量原始字段遗漏，新增/修改任务可用严格模式阻断。`getRegisteredJobDefinition()` 是执行和恢复路径的已注册任务入口，未知任务不得由默认配置静默接管。
- **日期策略消费**：`datasetDependencyState()` 只能通过 `resolveDatasetPartitionDate()` 计算依赖分区日；`partitionDatePolicy=business_date` 使用槽位业务日，`previous_trading_day` 使用前一开市日，`expected_data_date` 复用任务 `expectedDataDate()`。非法策略失败关闭，不得静默退回同日。
- **告警证据注册**：`ALERT_EVIDENCE_POLICIES` 明确故障类型、作用域和是否允许自动收敛；未知类型、通知型记录和人工类型只能保留并人工核对。来源具体接口查询同时按 SQL 参数和结果字段精确过滤 `api_name`，`source:*` 只服务来源级告警。
- **规则追踪与真实回归**：`governance/rule-traceability.json` 登记规则 ID、实现符号、测试文件、责任人和状态；`scripts/generate-job-matrix.js` 将其并入现有任务矩阵并在生成/校验时检查文件和符号。`server/test/inc-0010-regression.test.js` 使用测试库真实写入 `ops.dataset_partitions`，验证非默认 `previous_trading_day` 依赖只命中正确分区。正式决策见 `docs/decisions/ADR-0003-告警与任务治理门禁补强.md`。
...
| 新股历史（`ipo_history`） | `ipo_history_sync.py` 是 `new_share` 唯一采集者：18:00 和 19:30 两个 `core` 阶段各自按增量窗口获取并保存原始响应，upsert 申购日、上市日、名称和代码并执行基础事实门禁；16:30 `prediction_ready` 只针对下一交易日申购或上市的新股检查预测资料缺口，先读统一标准层、再定向补齐；19:35 `enrichment` 继续补历史缺口和首日表现，不再重复刷新 `new_share`，并跳过同日已尝试的目标代码；下游暴露以 `business_exposure` JSONB 留存证据、权重和可信度 | `/api/ipo/history`、打新历史页面、打新日历/日报和预测校准（均读已入库事实）；A 股历史接口返回 `total + limit + offset` 分页；历史接口同时返回 `history_stage`、`field_status`、`data_as_of` 及预测基础值/风口修正/可信度；新股历史中的“预测涨幅”在有预测和证券代码时链接至 `ipo-report.html?code=...` 打新详情，详情优先实时读取 `ipo_history` 和预测表，避免旧日报覆盖新补数据；详情在发行阶段同时展示 XGBoost 模型、模型基准涨幅、热度赛道、动态热度系数、样本数和受控赛道修正，并展示建议评分逐步计算和带单位/派生含义说明的模型输入/补位字段 | 启动追赶 + 60天重叠窗口；空响应/异常不覆盖旧值；核心阶段不调用 CNINFO 等资料接口且只以基础事实质量发布；资料补全不设业务条数上限，当前发行记录优先，历史缺口按统一 Guard 的真实来源保护线和任务异常止损线续跑，并用字段状态与尝试日期保证可恢复；行业、主营业务和业务暴露为发行阶段增强事实，行业 PE 为可低频复查的可选字段；缺失时优先解析交易所发行公告及投资风险特别公告，交易所列表未命中时走既有 CNINFO `hisAnnouncement` 备源；解析中证行业最近一个月静态 PE 时保存公告口径基准日及文档身份/URL，未盈利状态必须有公告正文证据，未知状态不推断为亏损；发行结果/中签率公告存在时，交易所主源解析网上有效申购倍数和最终中签率并留存公告证据；仅在结果字段确实尚未公布或尚未解析成功时进入 `pending_not_due`，不伪造事实；预测在发行公告阶段即生成，缺少结果字段时记录 `model_imputed_fields`、`result_fields_pending`，并输出按样本外误差扩大的可能区间；基础质量不通过不发布，增强缺口显示待补全；招股书检索按返回总数翻页，主营业务完整文本入库，仅在页面摘要时截短；风口校准使用近365天非北交所新股的稳健中位数直接除以同期市场基准，不做样本收缩或最终系数上下限；业务可信度只描述赛道分类证据，不再折扣赛道系数 | 股数：万股；发行价：元；募资/公开发行市值：亿元；中签率：%；业务暴露权重：0~1；风口可信度：0~1；质量状态：`complete/missing/pending_not_due`；字段状态：`value/retryable/source_unavailable/pending`；分区质量：`passed`；增强状态：`passed/partial/waiting_external/failed` | 工作日16:30上市前预测缺口检查 + 18:00核心事实 + 19:30核心复核 + 19:35全量资料补全；18:05/19:45日报消费数据库中的上市前资料 |
...
| 港股 IPO 动态申购信号（`hk_ipo_subscription_signals`） | `server/services/hkIpoMarketSignals.js` 将公开页时点信号写入 `analytics.hk_ipo_market_snapshots`；迁移 161 支持多时点快照，迁移 162 登记 HKIPOx 匿名页面；现只解析“今日申购”的认购倍数，按 `subscription_estimate` 写入，采集时间入库、来源观测时间为空 | `/api/ipo/calendar`、`/api/ipo/history`、`/api/ipo/report/code` 只读 `quality_status='valid'`，HKIPOx 以最近一次相同值观测的 `last_seen_at` 或最新快照采集时间按 2 小时派生 `fresh/stale`；`datasetPartitionRegistry` 按 `HK + signal_type=subscription + data_date` 登记分区 | 页面标为 HKIPOx 参考申购倍数；不填预计孖展金额或最终超购；来源失败保留历史值并降级动态信号，不阻断 `hk_ipo_facts`；只在现有 08:30/19:40 两个 IPO 阶段采集，盘中加密采集等待连续 5 个工作日稳定性核验 | 倍数：标准数值；时间：本地首次采集时间，来源观测时间为空 | 版本 `0.8.1.83` 已部署、迁移 162 和生产只读 API/页面验收通过；17:21 首次定时采集未到时，来源快照为 0，真实倍数待 19:40 首轮任务验收；页面使用条款尚未查明 |
| 港股 IPO 上市后日线（`market.daily_bars`） | `server/jobs/hkIpoSync.js` 的 IPO enrichment 使用现有腾讯港股 K 线入口，统一写入 `instrument_id` 并保留来源与原始摘要；Tushare 实现仍保留给兼容调用 | 港股 IPO 首日/五日覆盖审计、`hkIpoBacktest` | 腾讯行按 `(instrument_id,trade_date,source_id)` 幂等保存；缺行情或失败不覆盖有效数据，门禁从标准层重新计算 | 价格：HKD；成交量：股；成交额：HKD（腾讯源无成交额时保留空）；来源：`tencent` | IPO enrichment 不再调用 Tushare `hk_daily`；不在页面请求中回源；正式建议门禁仍受双环境探针与回测条件约束 |
...
| 港交所交易日历（`market.trade_calendar.exchange='HKEX'`） | `hkTradeCalendarSync` 当前仍使用 Tushare `hk_tradecal` 写入交易日、开市标记和原始载荷；HKEX 网页替代入口尚未完成采集器核验 | 港股 IPO 日期窗口和后续港股日线任务 | 入口失败不覆盖旧日历；与 SSE 交易日历分区保存 | 日期：YYYY-MM-DD；开市：布尔 | `hk_trade_calendar_sync` 工作日 07:50；免 Token 替代仍待实现 |
...
- **字段语义**：`positions.price` = 当前行情价（只由行情刷新更新）；`positions.cost` = 移动加权成本（由账本事务维护）。新持仓先以 `0` 表示待行情刷新，交易成交价不写入 `positions.price`。
...
- **交易后行情刷新（2026-09-20）**：新增/修改交易、期初建仓和持仓调整事务提交后，服务端立即对受影响证券调用统一批量行情入口；复用 60 秒全局行情缓存，缓存过期或缺失才访问上游。行情返回的 `quote_time` 决定实际价格时点：盘中为最新成交，收盘后/休市为上一次真实成交（例如周六录入取周五收盘），不按周末或“最近完整交易日”推算。有效行情只更新 `positions.price`，不改变成交价和成本；行情失败不回滚交易，新持仓保留 `0` 并显示待刷新状态。
...
**交易录入币种显示（2026-09-21）**：手动录入页面根据细类动态显示币种；港股显示港币 HK$，美股显示美元 US$，其他品种显示人民币 ¥。交易历史的价格、成交额和费用同步显示对应币种；代码分类将字母证券代码识别为美股，避免误判为港股。
...
2026-09-20 数据源整改确认：AiPO 认证不可用不阻断 HKEX 官方事实、最终公开发售超购和上市后表现；申购期认购/孖展属于可降级第三方时点信号，只有通过授权、双环境结构探针并在真实招股窗口取得正样本的来源才可启用盘中槽位。LiveReport 当前只确认到 SPA 应用壳，不能按静态 HTML 数据源使用。现有 `analytics.hk_ipo_market_snapshots` 继续作为唯一市场信号表，但日级唯一键会覆盖同日多次采集，后续须兼容扩展为按“业务日 + 来源内容/上游时刻”幂等的多时点快照，跨业务日相同值仍各自保留；`ipo_history.oversubscribe_multiple` 仍只保存配售结果中的最终官方倍数。HK 日历还须把完整公众招股窗口按上海业务日与查询窗口求交并钳制分组日，窗口任一端为空时不生成申购事件，但上市等其他事件继续保留；动态源失败不得阻断 `hk_ipo_facts`。完整研发和验收边界见 [`港股IPO数据源与实时申购信号整改研发方案_20260920.md`](港股IPO数据源与实时申购信号整改研发方案_20260920.md)。

2026-09-20 P0 实现与生产验收：迁移 161 已将港股市场快照改为按来源记录哈希支持多时点幂等，并补齐来源观测时间、孖展字段、`signal_kind` 与 `quality_status`；`/api/ipo` 的 HK 日历按上海业务日与完整招股窗口求交，只有 `upcoming/open` 未截止窗口生成申购事件，上市事件仍单独保留；HK 历史/详情日期也统一按上海时区格式化。`hkIpoSync` 先重算最终存储列对应的 `data_completeness`，动态信号通过独立 `hk_ipo_subscription_signals` 诊断发布，失败不计入核心事实失败集合。提交 `828f76a` 已完成标准生产部署，迁移 161、加密备份 `/var/backups/portfolio/portfolio_20260920_161441.sql.gz.gpg`、Web/Worker/健康检查定时器、`/health`、`/ready`、港股日历接口和静态脚本验收通过；第三方来源准入及附加盘中槽位继续关闭，未执行历史动态补档。

2026-09-20 港股历史页现场复核：线上 181 条港股记录中，中文名 177 条、公开发售起止日期各 172 条、配售日期 166 条、最终发行价 172 条、最终超购 168 条；动态申购/孖展信号当前无有效快照。对截图对象核对后确认 06700/09856 的官方配发公告尚未到招股书规定截止时间，06727 的 9 月 18 日结果在当日 19:40 槽位之后发布。版本 `0.8.1.74`（提交 `f4f9e26`）已生产发布，读取 `offer_phase`、完整招股窗口和有语义的待公告状态均已由线上静态脚本与接口核对通过，不以空值冒充实时或最终事实。
...
| 可转债公告、生命周期、上市流通规模、下修监控与动机评分 | 持有人/质押输入收盘日次日 07:20；统一公告任务 07:40 按收盘日次日口径采集盘后公告，17:30 按工作日口径补当天公告、唯一调用一次 `cb_issue`，并在同一槽位增量补上市流通规模；同一批官方公告在本地路由为生命周期、下修/转股价和强赎事件，原独立 07:45 强赎任务退出。公告及生命周期按 `ops.sync_cursors` 回扫 3 日重叠窗口；行情完成后 08:15 串行完成 `reset-v2` 下修进度和 `motive-v1.1` 评分。公告发现主链严格走上交所/深交所，失败或分页不完整保留游标重试；上市流通明细补全优先交易所，交易所未形成结果时才切 CNINFO 兜底。上市公告书前十持有人、发行结果和流通规模按实际来源幂等 upsert，旧巨潮事实在正常补全时重新核验并覆盖；`cb_issue`成功且明确为空时推进已核验水位，外部失败或生命周期事件质量不通过时不覆盖旧事实、不推进水位；历史流通规模失败保留降级诊断，但下一交易日目标债券缺失时返回 `DATASET_INCOMPLETE` 并进入现有任务重试，日报在对应数据集分区发布前不生成 | 统一公告原始记录 → 生命周期/上市流通规模/下修/强赎本地分类 → `reset-v2` 进度与 `motive-v1.1` 评分快照；评分只读库、不调用外部 API；接口显式返回 `expected_trade_date`、`stale` 与质量诊断 |
| IPO事实同步 | 工作日 16:30 上市前预测缺口检查、18:00 基础事实、19:30 核心复核、19:35 全量资料补全；启动时检查水位 | 16:30 只处理下一交易日目标并按目标代码统计剩余字段，不要求核心分区提前发布；18:00 和 19:30 发布质量通过的 `ipo_history` 核心分区；19:35 按自身资料与首日表现剩余项判定阶段完成，只依赖已发布且质量通过的核心分区，不重发核心分区。失败对象、剩余字段和 Guard 恢复原因进入现有重试/人工分流；缺字段继续显示待补全 |
| 港股 IPO 事实同步 | 08:30 / 18:10 / 19:40 | 港交所新上市表、披露易和官方文件分段采集，写入 `market_code='HK'` 的事实和 `HK` 分区；每轮先重算最终存储列对应的完整度。第三方动态源未通过授权与来源准入时不执行请求，动态信号标记 `not_admitted/not_run` 并显示不可验证，不产生每日故障告警；空结果不解释为无新股，未通过双环境与回测门禁时仅展示事实 |
| 港交所交易日历 | 工作日 07:50 | 只写入 `market.trade_calendar(exchange='HKEX')`；入口未确认时安全跳过并保留上一份数据 |
| 打新日历与日报 | 工作日 18:05；资料补全后 19:45 刷新 | 只依赖同业务日 `ipo_history`、`bond_issuance_events`、`bond_listing_liquidity` 分区已发布且基础质量通过，不绑定 `ipo_history_sync` 或可转债任务的整体成功状态，也不再要求只读日报返回任务级日期水位；Runner 按业务日计算下一个交易日作为“明日建议”目标日；18:05/19:45 均只读数据库生成，发布前与日历证券集合对账，失败保留上一份有效结果 |
...
任务定义统一声明 `externalSources`、`externalApis`、`producesDatasets`、`consumesDatasets`、`datasetDependencies`、`retryPolicy`、`catchupMode` 和 `dataDatePolicy`。联网任务的 `maxExternalCallsPerRun` 保持 `NULL`，`slotExternalCallsLimit` 和 `dailyBudget` 不再填入内部估算数字；本地纯计算任务保留 0 次外部调用约束。完整矩阵由 `scripts/generate-job-matrix.js` 生成并由 `check:knowledge` 校验；原始契约审计不把默认值补齐误认为显式声明。可执行状态为到期的 `pending`、带未来 `next_attempt_at` 的 `failed`，以及外部来源恢复后到期的 `waiting_external`；`succeeded`、`degraded` 和未解除的 `blocked` 不进入业务执行队列。任务依赖只认同业务日且早于下游计划时刻的最近上游实例，数据集依赖必须通过统一日期解析器命中指定分区和要求的质量状态；未知任务和非法分区策略失败关闭。成功但水位落后只显示为 `degraded`，后续由调度同步入口只读数据库重验，不重新调用 Runner。外部任务先做数据库游标/业务表新鲜度门禁，严格发布任务还必须确认目标分区已发布，`fresh` 结果的外部调用数为 0；多来源任务按所有必要来源的最慢水位确认。手动补跑默认同样受门禁，只有管理员显式 `force=true` 才能强制重拉。`latest_only` 任务补跑窗口只生成一个最新计划实例，按日分区的收盘数据仍按缺失代码补齐。下修动机输入不再按 10 次调用切批，默认完整遍历当期实际候选；其他联网任务同样不使用内部调用数作为完成边界。真实调用量继续写入运行记录供审计；真实上游限流进入带恢复时间的等待，任务超时或无进展才进入续跑/人工阻塞。
