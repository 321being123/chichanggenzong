...
任务水位校验会先归一化业务日期；需要上一交易日数据的任务显式声明日期策略，静态或低频配置同步任务按执行结果判断，不强制要求当日水位。同步任务完成时优先读取数据库游标，若游标刚写入尚未可见则使用结果中的 `data_as_of/window_end/trade_date` 兜底；已确认新鲜的 `fresh/already-ran-today` 跳过结果直接记为成功，避免反复补偿。重新校验成功会自动恢复对应历史告警。套利 PDF 解析对超过大小限制的永久失败会直接记为耗尽，避免无效重试；其他解析失败仍按有限退避重试。
