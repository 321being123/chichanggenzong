...
- 历史证券 ID 审计脚本 `server/scripts/auditInstrumentMerges.js` 负责候选和影响量；迁移125在备份和事务保护下按标准代码唯一映射迁移全部业务外键并物理删除历史主档，同时清除无效 `nan/HTML` 代码、孤立公司和旧候选记录。标准主档只允许一条发行公司关系，官方 `ts_code` 关系优先保留；迁移完成后以唯一索引和格式约束阻止裸代码及同数字重复主档再次写入。
