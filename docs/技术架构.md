...
- 当前生产版本为 `0.7.0.23`（承接 `0.7.0.22`，提交 `407eb4e`）；下修监控启动补漏已识别 v7 解析器遗留的到期日锁定并触发库内官方 PDF 重解析，迁移132/133、Web/Worker/健康检查定时器及 `/health` 已完成线上核对；当前最新行情日为 `2026-09-02`，下修接口等待 `2026-09-03` 行情分区时按规则标记 `stale=true`。
