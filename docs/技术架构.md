...
> 最后核对日期：2026-09-22。应用版本以根目录 `package.json` 的 `appVersion` 为准。
...
| IPO事实同步 | 工作日 16:30 上市前预测缺口检查、18:00 基础事实、19:30 核心复核、19:35 全量资料补全；启动时检查水位 | 16:30 先从统一标准层补行业和行业 PE，再仅补下一交易日申购/上市新股仍缺失的行业、主营业务和业务暴露；18:00 和 19:30 调用一次 `new_share`，19:35 继续补历史缺口和首日表现并跳过同日已尝试目标；各阶段按行提交已完成进度，遇外部 Guard 错误保留结构化原因进入等待/人工分流；发行结果未公布字段明确显示待公告，预测同时显示发行公告版可能涨幅和区间；行业/赛道缺失显示待补全，不伪装成“其他赛道” |
| 港股 IPO 事实同步 | 08:30 / 18:10 / 19:40 | 港交所新上市表、披露易和官方文件分段采集，写入 `market_code='HK'` 的事实和 `HK` 分区；每轮先重算最终存储列对应的完整度，动态申购信号单独登记 `hk_ipo_subscription_signals` 并只作为可降级诊断；空结果不解释为无新股，未通过双环境与回测门禁时仅展示事实 |
