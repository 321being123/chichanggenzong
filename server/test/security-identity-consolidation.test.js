...
(async () => {
  ...
  assert.strictEqual(await resolveCanonicalCode('920002.SH', 'stock'), '920002.BJ');
  assert.strictEqual(await resolveCanonicalCode('600519.BJ', 'stock'), '600519.SH');
  assert.strictEqual(await resolveCanonicalCode('000001.BJ', 'stock'), '000001.SZ');
  ...
  assert.strictEqual(await resolveCanonicalCode('113575.SZ', 'convertible_bond'), '113575.SH');
  assert.strictEqual(await resolveCanonicalCode('123175.SH', 'convertible_bond'), '123175.SZ');
  ...
  assert.ok(source.includes('canonicalizeSecurityCode(canonicalCode, assetClass)'), '主档写入必须校验数字代码与后缀是否冲突');
  ...
  assert.ok(migrationsSource.includes("126_normalize_stock_code_suffix") && migrationsSource.includes('CREATE OR REPLACE FUNCTION normalize_stock_code'), '数据库兜底函数必须同步校验后缀冲突');
