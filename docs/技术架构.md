...
| 新股历史（`ipo_history`） | `ipo_history_sync.py` 是 `new_share` 唯一采集者：18:00 核心阶段增量 upsert 申购日、上市日、名称和代码，并优先补齐目标申购日的行业、行业 PE、主营业务和产品→下游业务暴露；19:30 先重新刷新 19:00 后的 `new_share` 发行公告，再重试发行资料、历史缺口和首日表现；下游暴露以 `business_exposure` JSONB 留存证据、权重和可信度 | `/api/ipo/history`、打新历史页面、打新日历/日报和预测校准（均读已入库事实）；A 股历史接口返回 `total + limit + offset` 分页；历史接口同时返回 `history_stage`、`field_status`、`data_as_of` 及预测基础值/风口修正/可信度；新股历史中的“预测涨幅”在有预测和证券代码时链接至 `ipo-report.html?code=...` 打新详情，详情优先实时读取 `ipo_history` 和预测表，避免旧日报覆盖新补数据；详情在发行阶段同时展示 XGBoost 模型、模型基准涨幅、热度赛道、动态热度系数、样本数和受控赛道修正 | 启动追赶 + 60天重叠窗口；空响应/异常不覆盖旧值；资料补全不设业务条数上限，当前发行记录优先，历史缺口按统一 Guard 的真实来源保护线和任务异常止损线续跑，并用字段状态与尝试日期保证可恢复；行业、主营业务和业务暴露为发行阶段必需事实，行业 PE 为可低频复查的可选字段；结果公告尚未给出的上市日期、中签率、超额认购倍数进入 `pending_not_due`，不伪造事实；预测在发行公告阶段即生成，缺少结果字段时记录 `model_imputed_fields`、`result_fields_pending`，并输出按样本外误差扩大的可能区间；质量不通过不发布；招股书检索按返回总数翻页，主营业务完整文本入库，仅在页面摘要时截短；风口校准使用近365天非北交所新股的稳健中位数，相对同期市场基准并按样本数收缩，系数限制在 `0.80~1.50` | 股数：万股；发行价：元；募资/公开发行市值：亿元；中签率：%；业务暴露权重：0~1；风口可信度：0~1；质量状态：`complete/missing/pending_not_due`；字段状态：`value/retryable/source_unavailable/pending`；分区质量：`passed` | 工作日18:00核心事实 + 19:30公告刷新/资料补全；18:05日报只消费当天已发布分区 |
