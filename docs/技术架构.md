...
补充（2026-09-08）：公告增量同步把 PostgreSQL 日期游标归一为上海自然日后，能覆盖前一日公告；来源级 `BUDGET_WAIT` 的 `recoverAt` 同时贯穿 PDF 重解析和公告同步，巨潮日额度耗尽时两类任务统一等待次日恢复。
