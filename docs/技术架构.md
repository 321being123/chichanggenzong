...
> **单一任务注册清单（WORKER-01，2026-08-05）**：所有后台任务的「启动补漏 + 周期调度」统一收敛到 `server/scheduler.js` 的 `SCHEDULER_REGISTRY`，`server/app.js`（未设 `DISABLE_SCHEDULER` 时）与 `server/worker.js` 共用同一份，避免漏注册。注册表含 7 个周期任务（marketCloses / marketVolatilitySync / bondSafetyRefresh / stockAnalysisRefresh / convertibleBondRefresh / ipoCalendarRefresh / hkTradeRulesSync）+ 4 个启动任务（holidaySync / backfillCloses / indexBaseline / hkTradeRulesStartup）。上表为该清单的时间口径。PM2 拆分配置见 `deploy/ecosystem.config.js`（新增 `portfolio-worker`，Web 过渡期仍默认运行调度，启用拆分时给 Web 设 `DISABLE_SCHEDULER=1`）。
