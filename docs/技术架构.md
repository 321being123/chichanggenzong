...
> 最后核对日期：2026-09-02。应用版本以根目录 `package.json` 的 `appVersion` 为准。
...
| `/api/bond-safety` | 安全性快照、导出、刷新；公开快照的 `configured` 仅表示外部适配器配置状态，不改变数据库快照读取 | 快照/导出公开只读；刷新需管理员 |
...
| `ops` | 数据源、采集批次、原始响应、同步游标、质量问题、数据集发布分区、外部请求预算和按凭据/接口隔离的熔断 | `data_sources`、`ingestion_runs`、`raw_records`、`sync_cursors`、`data_quality_issues`、`dataset_partitions`（数据日期/发布时间/新鲜度）、`external_call_budgets`（只计数，迁移076）、`external_circuits`（Token 指纹＋接口，迁移078）；`source_endpoint_policies` 为后续接口策略表 |
...
| 新股历史（`ipo_history`） | `ipo_history_sync.py` 是 `new_share` 唯一采集者：18:00 核心阶段增量 upsert 申购日、上市日、名称和代码；19:30 补全阶段只处理行业、行业 PE、主营业务和首日表现 | `/api/ipo/history`、打新历史页面、打新日历/日报和预测校准（均读已入库事实）；历史接口同时返回 `history_stage`、`field_status` 和 `data_as_of` | 启动追赶 + 60天重叠窗口；空响应/异常不覆盖旧值；核心阶段按目标日核对接口集合与入库集合，发布 `ipo_history` 当天分区时记录申购/上市数量、证券集合哈希、采集批次和上市日缺口；质量不通过不发布 | 股数：万股；发行价：元；募资/公开发行市值：亿元；中签率：%；质量状态：`complete/missing/pending_not_due`；分区质量：`passed` | 工作日18:00核心事实 + 19:30非紧急补全；18:05日报只消费当天已发布分区 |
...
- IPO 统一事实链：`ipo_history_sync` 18:00 唯一调用 `new_share`，完成事实落库、内容质量校验并发布业务日 `ipo_history` 分区；`ipo_calendar_refresh` 18:05 同时依赖该任务成功和分区 `quality_status=passed`，再从 `ipo_history`、`market.trade_calendar` 与可转债标准事件表生成日历和日报。发布前对申购/上市的股票、转债四组证券集合逐项对账，不一致则任务失败、保留上一份有效日报并告警。19:30 仍复用 `ipo_history_sync`，但以 `enrichment` 模式补全非紧急字段且不发布核心日期分区；没有新增平行任务。
...
| 可转债安全性 | 收盘日次日 08:30 | 等待 08:00 可转债主链发布 `bond_master`、行情和估值分区后再计算；自动任务、人工补跑和后台刷新均只读已入库标准层，外部调用为 0；按计划 `businessDate` 取上一交易日；先统一 `cb_basic` 生命周期日期并排除标准发行事实中“定向/私募”未上市债券，再计算覆盖率；目标日数据未形成完整分区时向前最多回看 5 个开市日，覆盖率低于 `min(预期活跃债券数, max(90% 预期数, 100))` 不发布；水位校验取全部快照中的最新数据日期；无快照时启动后补一次 |
...
外部请求预算计数写入 PostgreSQL `ops.external_call_budgets`，Token＋接口熔断写入 `ops.external_circuits`，数据集并发锁使用 PostgreSQL advisory lock；三者职责分离，由 Node 服务和打新日报/新股历史 Python 子进程共用。Python Guard 默认开启，只有开发/测试显式 `EXTERNAL_CALL_GUARD=0` 才可关闭，生产环境即使设置 0 也强制开启；两端在同一预算表和 `external_budget:{source}` 锁下计数，并使用探测所有者、一次性令牌和5分钟租约回收异常退出的半开探测。预算日键始终取请求实际发生的上海自然日，不能被补跑任务的 `JOB_BUSINESS_DATE` 污染。当前系统内部保护线（2026-09-02复核）为：主、备用 Tushare 均为 `450次/分钟、5000次/日`，巨潮资讯 `20次/分钟、500次/日`；当前实际使用的腾讯行情不设置本系统分钟/日预算，仅保留上游异常处理；汇率、中债、港交所、公告/风控等其他经过 Guard 的来源仍使用通用 `60次/分钟、2000次/日` 兜底。450次/分钟为已确认官方500次/分钟的90%，保留50次/分钟余量，5000次/日按生产近期峰值1397次留出两倍以上余量，均不是上游官方日限额。配置仍可用 `<SOURCE>_PER_MINUTE_BUDGET`、`<SOURCE>_DAILY_BUDGET` 覆盖。内部预算到限使用 `BUDGET_WAIT` 等待下一分钟/交易日，不切备用 Token、不写 Token 熔断；真正上游 429/额度错误仍按接口级熔断和主备策略处理。业务错误保留 `api_name` 进入任务编排层；公告同步失败只记录 `last_error`，已有成功游标由数据库单调递增保护，不会因重叠窗口、旧进程收尾或并发写入回退。过期的 `job_runs` 租约由 Worker 自动回收，活动运行每分钟续租。

单接口限额策略目前**不单独存表**：`ops.external_call_budgets` 只保存“来源＋分钟/日窗口”的运行计数，`ops.external_circuits` 保存“Token 指纹＋接口”的熔断状态，二者都不是可维护的接口限额配置。若要长期管理 `new_share`、`daily_basic`、财务接口等单接口差异，应按专项方案新增 `ops.source_endpoint_policies` 策略表，至少保存 `source_id`、`api_name`、`credential_profile`、官方最低积分、权限模式、官方分钟/日限额（未知时允许 NULL）、内部安全分钟限额、并发/间隔、行数上限、重试策略、启用状态、最近核验时间和备注；只保存不可逆 Token 指纹或凭据角色，不保存原 Token。策略表落地时必须同步把预算计数键和 `consume_external_call_budget` 原子函数扩展到“来源＋接口＋凭据”，否则只建表不会真正限制调用。本次先完成数值调整，不提前引入这项数据库和运行时改造；待主备 Token 的接口权限矩阵和各接口实测调用量确认后再实施。
...
- 金融数据：`TUSHARE_TOKEN`、可选的 `TUSHARE_BACKUP_TOKEN`、`ANNOUNCEMENT_TUSHARE_FALLBACK`、腾讯行情 TTL、可转债同步并发和估值参数；外部请求保护可用 `<SOURCE>_PER_MINUTE_BUDGET`、`<SOURCE>_DAILY_BUDGET`（例如 `TUSHARE_PER_MINUTE_BUDGET`、`TUSHARE_BACKUP_PER_MINUTE_BUDGET`、`CNINFO_DAILY_BUDGET`、`TENCENT_PER_MINUTE_BUDGET`）调整预算，未配置时使用代码内安全默认值。后台外部 API 参数保存在 `platform_config.external_api_configs`，凭据加密且接口只返回掩码；IPO Python 子进程由 Node 注入后台当前的主备凭据与模式，不单独保存密钥。
