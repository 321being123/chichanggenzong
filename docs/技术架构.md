...
外部请求预算计数写入 PostgreSQL `ops.external_call_budgets`，Token＋接口熔断写入 `ops.external_circuits`，数据集并发锁使用 PostgreSQL advisory lock；三者职责分离，由 Node 服务和打新日报/新股历史 Python 子进程共用。Python Guard 默认开启，只有开发/测试显式 `EXTERNAL_CALL_GUARD=0` 才可关闭，生产环境即使设置 0 也强制开启；两端统一调用 `ops.reserve_external_call(source, api_name, credential_profile, credential_fingerprint)`，在 `external_budget:{source}:{credential_fingerprint}` 锁下按来源＋接口＋凭据原子计数，并使用接口并发槽和最小间隔运行态。预算日键始终取请求实际发生的上海自然日，不能被补跑任务的 `JOB_BUSINESS_DATE` 污染。当前默认策略（2026-09-02复核）为：主、备用 Tushare 均为 `450次/分钟、5000次/日`，巨潮资讯 `20次/分钟、500次/日`；当前实际使用的腾讯行情不设置本系统分钟/日预算，仅保留上游异常处理；其他经过 Guard 的来源使用各自策略表中的来源级兜底值。内部预算到限使用 `BUDGET_WAIT` 等待下一分钟/交易日，不切备用 Token、不写 Token 熔断；真正上游 429/额度错误仍按接口级熔断和主备策略处理。预算策略、官方权限、重试/超时/空结果规则通过后台“外部 API 主备”的接口策略面板维护；Token 只保存加密值，策略表只保存角色和不可逆指纹。

单接口限额策略已由迁移127落地到 `ops.source_endpoint_policies`：按 `source_id`、`api_name`、`credential_profile` 保存官方最低积分、权限模式/状态、官方分钟/日限额（未知允许 NULL）、内部分钟/日限额、并发、最小间隔、行数上限、超时、重试/空结果策略、启用状态、最近核验时间和备注；仅保存不可逆凭据指纹，不保存原 Token。`ops.source_endpoint_runtime` 保存接口＋凭据的最小间隔状态，`ops.external_call_budgets` 的主键已扩展为“来源＋接口＋凭据指纹＋窗口”，Node 与 Python 均通过 `ops.reserve_external_call` 原子执行接口级和凭据跨接口累计限流。精确策略优先于来源级 `api_name='*'` 兜底，官方已知上限会自动成为硬上限；权限拒绝/未配置直接阻断，分钟/日/并发/间隔达到保护线进入等待。后台接口策略面板支持按主/备凭据分别调整内部规则，Token 轮换时自动更新指纹并重置待核验状态。
