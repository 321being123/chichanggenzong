...
- 当前生产版本为 `0.7.0.26`（提交 `b191b42`）；已完成迁移134、财务增量任务和安全性质量门禁的代码部署。待发布版本 `0.7.0.27` 增强专项回填的缺口续跑和确定性批次顺序，默认跳过已完成公司，只有显式 `--force` 才重跑；生产回填与后续安全性刷新须在该版本部署后执行。下修监控启动补漏已识别 v7 解析器遗留的到期日锁定并触发库内官方 PDF 重解析，迁移132/133、Web/Worker/健康检查定时器及 `/health` 已完成线上核对；当前最新行情日为 `2026-09-02`，下修接口等待 `2026-09-03` 行情分区时按规则标记 `stale=true`。
