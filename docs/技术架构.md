...
- 当前生产版本为 `0.7.0.24`（提交 `11a99a5`）；已完成迁移134、财务增量任务和安全性质量门禁的代码部署。专项回填范围修正版 `0.7.0.25` 仅针对最新安全性行情中的有效可转债正股，并保留 `--offset` 断点续跑；生产回填与后续安全性刷新须在该版本部署后执行。下修监控启动补漏已识别 v7 解析器遗留的到期日锁定并触发库内官方 PDF 重解析，迁移132/133、Web/Worker/健康检查定时器及 `/health` 已完成线上核对；当前最新行情日为 `2026-09-02`，下修接口等待 `2026-09-03` 行情分区时按规则标记 `stale=true`。
