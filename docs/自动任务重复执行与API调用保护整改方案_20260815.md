...
> 当前状态（2026-09-01执行批次）：任务契约、预算等待语义、Python Guard 默认开启（含探测租约）、核心分区表、运行时代码映射、共享收盘采集和全A股股票标准行情分区已落地；迁移119～124已完成股息率单位修复、分区水位初始化、历史股票类型/公司关系归一，并将 Node/Python 预算扣减统一到数据库原子函数。财务三表的持续增量已复用现有标准层，历史证券 ID 物理迁移仍待人工审批与分批对账。
> 生产授权执行记录（2026-09-01）：已部署提交 `c989bfb`（版本 `0.7.0.0`），部署前数据库备份为 `/var/backups/portfolio_before_0_7_0_0_20260901_191425.dump`（SHA-256：`ef153e23cf879f5d803c4923e2e428553b626b2c56849b614357e91932d38836`）。生产权限探测共 48 项：18 项可用或空结果可接受，30 项受限；主 Token 的 24 项探测均未发出上游请求（多数因统一 Guard 当日预算保护，接口项受既有熔断状态拦截），备用 Token 的标准接口可用/空结果可接受 18 项，4 个 `_vip` 接口无权限，`cb_price_chg` 与 `top10_cb_holders` 受限。维护窗口已将 139 条可直接规范化的裸股票代码更新为标准代码；剩余 4,591 条需合并候选，未执行物理合并或删除。生产候选审计已留痕 4,789 组（名称一致 4,624 组、需人工核对 165 组、受影响引用 9,350 行），旧 `test_guard` 熔断记录为 0。影子对账尚未完成：部署前最后一次共享收盘任务（2026-09-01）记录 281 次 Tushare 调用，新版本需从下一交易日起连续观察 3 个交易日后再下结论。
...
| 数据分区发布 | 已建立 `ops.dataset_partitions`，可转债主采集链路发布 `bond_daily`、`stock_daily`、`stock_valuation`、`stock_adj_factor`、`stock_suspend_calendar` 五个核心日分区，安全评分和估值另发布快照分区；其他采集器尚未全部接入 | `ops.dataset_partitions` 统一验证和发布 | 部分落地（核心链路已落地） |
...
| 历史证券 ID 审计 | 迁移118建立 `core.instrument_merge_candidates`；`auditInstrumentMerges.js --write-candidates` 本地生成4,538组候选、当前4,810行外键引用统计（已排除候选表自身记录），默认不删除旧ID | dry-run → 映射 → 分表迁移 → 5日对账 | 已落地（只读审计），物理迁移未授权 |
...
1. 已生成只读 dry-run 报告：本地候选4,538组，其中4,367组名称一致、171组需人工核对，当前涉及外键引用4,810行（审计排除候选表自身记录）；报告包含候选主ID、旧ID、影响量和冲突原因。
2. 已建立持久化 `core.instrument_merge_candidates`（迁移118）保存审计结果；报告通过人工抽样后才允许建立 `old_instrument_id → canonical_instrument_id` 迁移映射。
3. 第一阶段只统一所有新写入到主ID，不更新、不删除旧ID；运行至少5个交易日并完成双读对账。
4. 第二阶段按表分批迁移外键，每批独立事务、独立备份和行数断言；发生一条无法解释的冲突即整批回滚。
5. 旧ID和旧表继续保留只读。只有全部链路稳定且用户再次明确同意后，才可另行归档或删除。
...
14. 只读验收入口 `npm.cmd run check:data-architecture`（`--json` 可导出机器可读结果），固定核查迁移、原子预算函数、供应商映射唯一性、有效转债正股关联、已发布分区、持久化任务队列和每日预算；权限探测、影子对账、历史 ID 合并与旧数据清理仍以受控待办单独列示。
