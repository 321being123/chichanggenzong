...
- 2026-09-05 只读核查发现：生产应用账号仍可写发布目录与 `.git`，系统 Python 与 IPO venv 依赖不同，两份受 Git 跟踪的 IPO 模型文件存在生产运行变更；这些生产权限与产物问题尚未上线整改。仓库 Nginx 模板已增加 `/api/` 优先 location，系统服务模板已加入文件系统隔离；仓库同时提供加密备份 service/timer 模板，但生产自动异地备份尚未启用。验收方案见本次安全架构审计。
