...
`company_financial_incremental_sync` 每日 06:30 按持仓、自选、可转债正股和 IPO 正股并集去重，读取 `disclosure_date`、缺口和重试游标后，仅对待同步公司请求 `income`、`balancesheet`、`cashflow`、`fina_indicator`，日常三表显式限定 `report_type=1`。Tushare `fina_indicator_vip` 响应不返回 `report_type` 时，按该接口的合并报表默认语义补齐标准层 `report_type=1`；普通接口仍必须以实际返回的 `report_type=1` 为准。数据先保存 `ops.raw_records`，再写入 `fundamental.financial_reports`/`financial_facts`；`source_version` 包含规范化原始报文内容哈希，空结果或失败不覆盖旧有效数据。日常任务使用 `latest_only`、批次上限和 `ops.sync_cursors` 续跑；一次性历史回填使用 `server/scripts/backfillCompanyFinancials.js`，默认仅预览，生产执行另需显式确认；应用模式默认只处理指定报告期仍有缺口的公司，支持 `--offset` 断点定位，只有显式 `--force` 才重跑已完成公司。
...
| 公司财务增量同步 | 每日 06:30 | 按目标公司并集、披露计划、报告期缺口和失败重试增量采集；写入 `stock_financial_reports`，无待办时以成功空跑结束且不发布伪分区；一次性历史回填不进入该槽位 |
...
- 当前生产版本为 `0.7.0.27`（提交 `fee70b8`）；已完成迁移134、财务增量任务、VIP 指标响应兼容、缺口续跑和安全性质量门禁部署。生产已按最新安全性行情回填308家正股的2026-03-31与2026-06-30两期四类报表，308/308家公司两期完整；安全性快照311条、财务覆盖率100%、评级覆盖率100%，`bond_safety_snapshot` 分区为2026-09-03且未过期。下修监控启动补漏已识别 v7 解析器遗留的到期日锁定并触发库内官方 PDF 重解析，迁移132/133、Web/Worker/健康检查定时器及 `/health` 已完成线上核对；当前最新行情日为 `2026-09-02`，下修接口等待 `2026-09-03` 行情分区时按规则标记 `stale=true`。
