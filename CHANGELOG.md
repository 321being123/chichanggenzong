...
## 2026-07-25 · 0.2.0.20
- 智能导入现在能正确识别场内基金、ETF、LOF 了：比如 160723 嘉实原油 LOF 不会再被误当成 A股股票，税费会按基金/ETF 口径（免印花税、免过户费）计算。

## 2026-07-25 · 0.2.0.19
- 智能导入（拍图片 / 传 Excel 自动识别交易和持仓）恢复正常：之前用的 AI 视觉模型被平台下线，导致识别全部失败；现已切换到平台当前可用的模型，识图和表格识别都恢复正常。
- 打新日报里的「赛道热度」表脚注和列名改成如实描述（之前写的是旧算法的说明，现已更正为真实的「新股首日涨幅 / 150」口径）。
- 修复 GitHub 自动检查报出的依赖高危漏洞：升级了 body-parser、uuid 和 Excel 解析相关的底层压缩库，消除拒绝服务风险，CI 检查现在能通过了。

## 2026-07-25 · 0.2.0.17
- 打新建议的「预计首日涨幅」现在会正确叠加赛道加成：之前热门赛道（如半导体）的新股漏算了赛道热度，预测偏低，现已补上。
- 赛道加成改成了直观的「基础涨幅 × 赛道系数」，不再用越热门放得越大的非线性公式——比如半导体系数2.0就是直接乘2，预测更可预期、不再虚高。
- 招股书取主营业务更准了：之前可能误抓到「招股说明书提示性公告」或募资段落，导致热门新股被误判赛道；现在固定取完整招股书里的真实主营业务和所属行业。

## 2026-07-25 · 0.2.0.16
- 修复打新建议里「赛道」下拉的「样本数」离谱的问题：之前把全市场所有股票都算进去了（比如半导体显示195个），现在只统计真正上市的新股只数（半导体19只），数字终于正常了。
- 赛道热度系数改成按「新股上市首日涨幅」计算，不再是乱七八糟的全市场老股涨幅——首日涨得越猛的赛道，热度越高，打新建议给的加成也更合理。
- 赛道匹配不再靠公司名字瞎猜：现在以招股书里的「主营业务 / 所属行业」为准；待上市的新股也能从招股书取到真实主营业务，准确判断它属于哪个赛道。

## 2026-07-24 · 0.2.0.15
- 打新建议的“预计涨幅”不再强制套用各板块的保底下限：之前科创板可能无依据地顶到600%以上，现已去掉，完全按模型和赛道热度计算。
- 修复打新建议里偶尔出现“预计涨幅0%”的问题：模型输出异常时会自动重新计算，不再乱显示0。
- 可转债打新历史恢复显示“股东配售率”真实数据：之前空缺或估算，现在从上市公告里取真实值。
- 可转债打新历史的“申购户数”：深市公告不公布真实户数，现已按配号数估算并在数据后标注“（估算）”；沪市仍用公告里的真实户数。

## 2026-07-24 · 0.2.0.14
- 修复打新历史列表里混入“定向可转债/私募可转债”的问题：这类债不是公开申购的，不该出现在打新历史里，已把它们从列表中过滤掉。

## 2026-07-24 · 0.2.0.13
- 修复可转债“上市涨幅”数值不准：之前所有债都只按上市首日收盘价计算，没有实现“连板到开板累计”的规则。已修正，现在首日涨停的会顺延到首个开板日计算累计涨幅（例如连续两天涨停后第三天开板的债，涨幅会如实反映到开板日）。

## 2026-07-24 · 0.2.0.12
- 修复打新历史里可转债名称显示成“XX发债”的问题：名称统一改为“XX转债”（与官方一致），之前误用了申购时的简称。

## 2026-07-24 · 0.2.0.11
- 修复打新历史里已上市的可转债仍然显示“申购日”的问题：补回了这些债券的上市日期，现在正确显示“上市日”。
- 修复部分可转债在打新历史里名称显示成“nan”乱码的问题，名称已全部补全。
- 修复可转债“上市涨幅”长期为空：原来的行情接口已失效，改用新的官方行情源重新统计了全部历史可转债的首日累计涨幅（连板到开板日的累计收益），统计口径保持不变。
- 打新历史可转债的评级、发行规模等资料一并补齐完整。

## 2026-07-23 · 0.2.0.10
- 持仓列表“今日盈亏”支持升序、降序排列，无有效数据的持仓固定排在末尾。
- 股票估值分位改为最低0%、最高100%的排名算法；负PE按正数在前、负数从小到大在后排序，20%、50%、80%分位改为随累计历史变化的曲线。
- 优化估值图异常高值显示：纵轴只由正常值确定，超出纵轴的数值在边界标色，主曲线保持连续且等宽。
- “十年三年均值增长”改为（近期三年均值－十年前三年均值）÷十年前三年均值。
- 股债分析的计算指标和模块标题新增虚线下划线及悬浮算法说明。
- 可转债安全性现金覆盖率改为流动负债、有息负债双口径，任一覆盖率不低于1即满足。
- “最近一年公司大事”改为“公司公告”，只展示正式公告；刷新改为回查最近7天并使用巨潮与交易所双源，公告源失败不再误报成功。
- 更新存在小站首页，新增股债分析入口和已上线功能概览，集中展示持仓收益、智能导入与打新研究能力。
- 募集用途与条款在到期赎回价后新增回售价，按100元本金加最快回售日的当期应计利息计算。
- 回售到账税前、税后收益率改为按最快回售剩余年限和回售价格计算，税后仅对利息部分扣除20%税。
- 修复多只转债共用年报或公告时转股价历史串债的问题，兼容上交所与深交所年报表格，并按债券名称独立解析、校验完整价格链。
- 正确区分下修、权益分派和回购注销引起的转股价调整；修复湘佳转债公告误取历史价格的问题。
- 最快回售触发日按回售期开始后的交易日累计计算；本计息年度回售机会已使用时，顺延至下一计息年度计算。
- 版本更新记录按日期合并展示，同一天只保留最后一个版本号。
- 恢复打新日报详情底部的预测跟踪统计和当前赛道热度系数，同时保留单个标的内容隔离。
- 热市且市场零破发时，新股统一给出顶格申购建议。
- 重新建立并持久化热门赛道成分股、60日涨跌幅和赛道系数；新股新债预测价格及上市后的实际价格、日期统一入库，方便持续更新预测准确度。

## 2026-07-22
- 日期格式化、日期加减、回售周期和查询区间统一按北京时间计算，避免不同服务器时区导致日期偏移。
- “股票分析”升级为“股债分析”，支持持仓、自选及任意代码的单只可转债分析，并以表格展示基本信息、安全性、债券价值、期权价值、正股指标及历史记录。
- 可转债数据改为首次全量入库、后续增量刷新；补充募集说明书、年报、评级报告及历史公告解析，完善募资用途、债券利息、转股价调整与不下修历史。
- 完善强赎、下修和回售计算，正确处理不下修锁定期、回售起始日、每计息年度回售机会及预计回售到账时间。
- 转股价历史新增原价、新价、下修到底/不到底等类型；基金持仓新增占剩余规模比例，债券利息明细突出当前计息年度。
- 修复纯债价值、债底溢价率、期权价值、资产负债率、股息率和评级展望等空值，并修复康医转债利息保障倍数旧缓存异常。
- 持仓列表标题和数据统一居中，新增今日盈亏（港股按汇率折算人民币），投资收益记录改为红涨绿跌。
