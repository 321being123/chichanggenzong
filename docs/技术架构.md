...
- **净值快照锚定与缺口保护（2026-09-10）**：后台 `nav_snapshot` 生成历史净值时，优先使用券商导入或 `manual_reconciliation` 的 `nav_position_snapshots` 作为持仓事实，只重放锚点之后的交易；历史港股代码别名按统一分类层归一。任一锚点持仓缺少收盘价/汇率时，账户结果返回 `partial`、缺失日期和代码，统一任务不得记录为成功。前端“今日涨跌”仅在上一条净值对应上一交易日时显示，存在交易日缺口时显示缺失状态，不把多日变化冒充当日变化。
