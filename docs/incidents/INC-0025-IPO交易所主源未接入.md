...
本地回归覆盖交易所优先、交易所失败才走巨潮、`92xxxx` 路由、巨潮熔断时 Tushare 末级回退和 `bse.cn` 来源分类；全量测试 131/131 通过、知识门禁及任务矩阵检查通过。生产 `.35` 版本、迁移 154、Web/Worker/健康检查服务和 `/health` 均验收通过；`.36` 发布后再核对历史槽位收口。
