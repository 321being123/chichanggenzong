...
2026-09-05 审计首批整改：AI 模型配置已复用 `externalApiConfig` 的 AES-256-GCM 保护，存量明文在启动时迁移，接口仍只返回掩码；知识分类、文章列表和评论入口已统一经过 `optionalLogin`，只信任已验证的 `req.authUser`；CSRF 已覆盖全部非 GET/HEAD/OPTIONS 方法（包括 PATCH）。邮箱变更要求当前密码，本人改密增加账号/IP 限流与 6–128 位长度约束。剩余服务器目录权限、异地备份恢复、Python 解释器统一和历史弱密码清理仍待生产维护窗口处理。具体证据与边界见 [安全架构审计方案](项目与服务器安全架构审计及整改方案_20260905.md)。
...
- 调度/脚本：`DISABLE_SCHEDULER`、`IPO_PYTHON_PATH`、`VALUATION_PYTHON`、`PYTHON`、`CALL_EVENT_PYTHON`。生产环境统一将这四个 Python 路径指向 `/opt/portfolio/ipo-report/venv/bin/python`，避免任务回退到系统解释器造成依赖漂移；本地可留空并沿用代码候选顺序。
...
- 2026-09-05 只读核查发现：生产应用账号仍可写发布目录与 `.git`，系统 Python 与 IPO venv 依赖不同，两份受 Git 跟踪的 IPO 模型文件存在生产运行变更；这些生产权限与产物问题尚未上线整改。仓库 Nginx 模板已增加 `/api/` 优先 location，系统服务模板已加入文件系统隔离；仓库同时提供加密备份 service/timer 模板，GPG 使用 `/var/lib/portfolio-backup/gnupg` 专用 root-only 工作目录，但生产异地同步与恢复演练仍需验收。验收方案见本次安全架构审计。
