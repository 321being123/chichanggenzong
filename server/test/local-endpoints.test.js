...
(async () => {
  ...
  if (!skip) {
    ...
    await checkAsync('净值导入：按日期 upsert 且版本提升', async () => {
      ...
      const expectedAuthoritative = (d1.positions || [])
        .filter(p => Number(p.quantity) > 0)
        .reduce((sum, p) => sum + Number(p.price || 0) * Number(p.quantity || 0), Number(d1.authoritativeCash || 0));
      assert.strictEqual(d1.authoritativeTotalAsset, expectedAuthoritative, '导入次日起应使用系统持仓绝对市值加导入后现金余额');
