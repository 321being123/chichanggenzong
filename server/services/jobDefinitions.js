...
const JOB_DEFINITION_SOURCE = [
  { jobCode: 'company_financial_incremental_sync', label: '公司财务增量同步', hour: 6, minute: 30, weekdays: false, catchupMode: 'latest_only', dataDatePolicy: 'latest_available', freshnessGate: false, requiresDataWatermark: false, strictDatasetPublication: true, dailyBudget: 0, deadlineMinutes: 120, timeoutMinutes: 30, sourceDescription: '当前上市可转债正股的Tushare披露计划与财务标准层；按公司、报告期增量入库', mayConsumeQuota: true, externalSources: ['tushare'], retryPolicy: 'external', retryDelaysMinutes: [15, 60, 240], maxAttempts: 4, slotExternalCallsLimit: 320 },
  ...
  { jobCode: 'hk_rate', label: '港币汇率更新', hour: 16, minute: 15, weekdays: true, deadlineMinutes: 90, dataDatePolicy: 'latest_available', freshnessGate: true, freshnessMode: 'max_age', freshnessMaxAgeMinutes: 1440, sourceDescription: '港币汇率接口', mayConsumeQuota: true, externalSources: ['exchange-rate'], retryPolicy: 'external', retryDelaysMinutes: [15, 60], maxAttempts: 3 },
  ...
  { jobCode: 'convertible_bond_announcement_history_sync', label: '可转债公告与生命周期同步', hour: 7, minute: 40, weekdays: true, afterTradingDay: true, additionalSchedules: [{ hour: 17, minute: 30, mode: 'calendar', afterTradingDay: false, weekdays: true }], catchupMode: 'latest_only', dataDatePolicy: 'latest_available', freshnessGate: true, requiresDataWatermark: true, reconcileByWatermark: true, strictDatasetPublication: true, deadlineMinutes: 240, timeoutMinutes: 120, importance: 'high', sourceDescription: 'Tushare发行事实与巨潮、上交所、深交所官方公告统一采集后按事件类型分发', mayConsumeQuota: true, externalSources: ['tushare', '巨潮资讯', '上交所', '深交所'], retryPolicy: 'external', retryDelaysMinutes: [15, 60, 240], maxAttempts: 4 },
  ...
  { jobCode: 'hk_trade_calendar_sync', label: '港交所交易日历同步', hour: 7, minute: 50, weekdays: true, catchupMode: 'latest_only', requiresDataWatermark: false, dailyBudget: 0, sourceDescription: '港交所交易日历探针确认后的官方/配置接口', mayConsumeQuota: true, externalSources: ['hkex', 'tushare'], retryPolicy: 'external', retryDelaysMinutes: [15, 60], maxAttempts: 3 },
  // 港交所官方入口使用独立 hkex Guard；dailyBudget=0 表示不挤占既有 Tushare 80 次总预算。
  { jobCode: 'hk_ipo_preopen', label: '港股 IPO 盘前事实同步', hour: 8, minute: 30, weekdays: true, catchupMode: 'latest_only', requiresDataWatermark: false, dailyBudget: 0, datasetDependencies: [{ datasetCode: 'hk_trade_calendar', scopeKey: 'HK', partitionDatePolicy: 'business_date' }], sourceDescription: '港交所主板/GEM 新上市页面、官方披露易、腾讯行情名称与利弗莫尔/华盛公开申购期信号', mayConsumeQuota: true, externalSources: ['hkex', 'livermore', 'vbkr-public', 'tencent'], retryPolicy: 'external', retryDelaysMinutes: [15, 60], maxAttempts: 3 },
...
const JOB_CONTRACTS = {
  'company_financial_incremental_sync': { externalApis: ['disclosure_date', 'income', 'balancesheet', 'cashflow', 'fina_indicator'], producesDatasets: ['stock_financial_reports'], consumesDatasets: ['stock_master', 'financial_disclosure_schedule'], maxExternalCallsPerRun: 80 },
  ...
  'hk_trade_calendar_sync': { externalApis: ['hk_trade_calendar'], producesDatasets: ['hk_trade_calendar'], consumesDatasets: [], maxExternalCallsPerRun: 1 },
  'hk_ipo_preopen': { externalApis: ['hkex_new_listings', 'hkex_prospectus', 'livermore_hk_ipo_current', 'livermore_hk_ipo_history', 'vbkr_hk_ipo_current', 'tencent_quote'], producesDatasets: ['hk_ipo_facts', 'hk_ipo_subscription_signals'], consumesDatasets: ['hk_trade_calendar'], maxExternalCallsPerRun: 8 },
  'hk_ipo_postclose': { externalApis: ['hkex_allotment', 'hkex_title_search', 'hkex_history_report', 'hkex_market', 'tencent_quote'], producesDatasets: ['hk_ipo_facts'], consumesDatasets: ['hk_trade_calendar'], maxExternalCallsPerRun: 16 },
