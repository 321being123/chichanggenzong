...
| 数据源 | 用途 | 失败策略 |
|---|---|---|
| Tushare Pro | 直连官方 A 股、财报、指数、可转债、交易日历、宏观数据及 `new_share` 新股发行清单；统一使用 `TUSHARE_TOKEN` 和官方 POST API | 页面优先读库；定时任务失败或可疑空结果保留旧数据并记录失败 |
...
- 金融数据：`TUSHARE_TOKEN`（直连 Tushare Pro）、腾讯行情 TTL、可转债同步并发和估值参数。
