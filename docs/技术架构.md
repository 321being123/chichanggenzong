...
> 2026-09-22 IPO 上市前预测时序修复（本地已验证，未部署）：`ipo_history_sync` 在 16:30 只针对下一交易日申购或上市且存在预测资料缺口的新股执行 `prediction_ready` 检查；先复用 `core.instruments.raw_data` 和 `market.daily_valuations`，只对仍缺失的字段定向补齐；打新日报仍按原计划 18:05 发布，18:00/19:30/19:35 原有事实同步、复核与全量补全继续保留。19:35 排除同日 16:30 已尝试的目标代码，避免重复请求。
