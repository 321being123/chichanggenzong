...
> 最后核对日期：2026-08-17。应用版本以根目录 `package.json` 的 `appVersion` 为准。
...
所有新增或修改的业务数据表格统一以“上市可转债”列表为视觉和滚动交互基准。当前参考实现为 `public/css/bond-safety.css` 的 `.bond-list-*` 样式和 `public/js/bond-list.js` 的表头、横向滚动同步逻辑；后续复用时应抽取到 `public/shared/style.css` 和共享脚本，禁止复制成多套页面私有实现。

| 项目 | 统一要求 |
|---|---|
| 容器 | 使用白色卡片，圆角 `10px`，轻阴影；工具栏位于表格上方并保持同一卡片视觉。 |
| 表头 | 高度不低于 `40px`，浅灰底 `#fafbfc`，文字 `#555`、`12px`、加粗、居中、不换行，底部使用浅色分隔线。 |
| 表体 | 字号 `12px`，单元格居中、不换行，纵向内边距约 `9px`；数字使用等宽数字，行间使用浅色分隔线，悬停行为 `#f7fbff`。 |
| 涨跌颜色 | 中国市场口径统一为红涨 `#d93025`、绿跌 `#137333`；零涨跌使用普通文字颜色。 |
| 宽表格 | 表格宽度使用内容宽度且不小于容器宽度；列不得为适配窗口而强制压缩或换行，超出部分通过横向滚动查看。 |
...
该规范约束视觉和通用滚动体验，不改变各业务表格的字段、金融公式、筛选条件和数据权限。现有页面在后续实际修改时逐步对齐，禁止为统一样式一次性重构全部表格。
...
| `analytics` | 计算指标、页面快照、可转债上市表现、列表/周期/估值和市场周期结果 | `metric_values`、`analysis_snapshots`、`convertible_bond_listing_performance`、`convertible_bond_list_metrics_daily`、`convertible_bond_valuation_daily` |
...
- **历史锚点输入**：`S_pos0` 优先使用导入日 `daily_prices` 与该日系统汇率；导入日没有行情或行情不完整时，取导入日前最近一个“全部非零锚点持仓都有有效收盘价”的日期，并将该组前收固化到导入日、标记 `broker_previous_close`/`system_previous_close`，禁止混用不同日期价格；缺少港股汇率时仍拒绝建立锚点。`/api/nav/import/preview` 返回实际行情日期和回退标记。
...
| 可转债主链 | 每日 18:00；次日 08:00 检查补跑 | 行情 → 周期 → 上市列表派生指标 → 估值 → 预警 |
...
Tushare 请求现在保留上游 HTTP/业务错误码并分类为网络、空数据、限额、权限或参数错误；主备凭据可在后台“全局参数 → 外部 API 主备”中加密保存，未配置后台值时回退到 `TUSHARE_TOKEN` / `TUSHARE_BACKUP_TOKEN`。自动模式下主 Token 遇到失效、权限、限额或熔断时切换备用 Token，主 Token 恢复后自动切回；也可在后台固定使用主 Token 或备用 Token，并按开关发送切换告警。参数错误不重试；429/配额错误按 Token 数据源隔离熔断，网络错误使用外部任务退避（15/60 分钟）。收盘任务把单次运行的内层请求和外层重试纳入同一个请求预算。
