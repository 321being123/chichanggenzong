...
2026-09-20 P0 本地实现：迁移 161 已将港股市场快照改为按来源记录哈希支持多时点幂等，并补齐来源观测时间、孖展字段、`signal_kind` 与 `quality_status`；`/api/ipo` 的 HK 日历按上海业务日与完整招股窗口求交，只有 `upcoming/open` 未截止窗口生成申购事件，上市事件仍单独保留；HK 历史/详情日期也统一按上海时区格式化。`hkIpoSync` 先重算最终存储列对应的 `data_completeness`，动态信号通过独立 `hk_ipo_subscription_signals` 诊断发布，失败不计入核心事实失败集合。上述仅完成本地代码、测试库和本地业务库验收，未执行生产迁移、生产补档或部署；第三方来源准入及附加盘中槽位继续关闭。
