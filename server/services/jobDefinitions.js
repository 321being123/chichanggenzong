...
const DEFAULT_JOB_OPTIONS = {
  ...
  // 槽位调用量只做审计；未知的累计调用上限保持 0，续批由任务超时、最长续跑时间和无进展保护停止。
  slotExternalCallsLimit: 0,
  continuationNoProgressLimit: 2,
  continuationMaxAgeHours: 24,
  modeExternalCallLimits: {},
  retryPolicy: 'local',
...
const JOB_DEFINITION_SOURCE = [
  { jobCode: 'company_financial_incremental_sync', label: '公司财务增量同步', hour: 6, minute: 30, weekdays: false, catchupMode: 'latest_only', dataDatePolicy: 'latest_available', freshnessGate: false, requiresDataWatermark: false, strictDatasetPublication: true, deadlineMinutes: 120, timeoutMinutes: 30, sourceDescription: '当前上市可转债正股的Tushare披露计划与财务标准层；按公司、报告期增量入库', mayConsumeQuota: true, externalSources: ['tushare'], retryPolicy: 'external', retryDelaysMinutes: [15, 60, 240], maxAttempts: 4 },
  { jobCode: 'bond_safety_refresh', label: '可转债安全评分', hour: 8, minute: 30, weekdays: true, afterTradingDay: true, deadlineMinutes: 120, dataDatePolicy: 'previous_trading_day', freshnessGate: true, strictDatasetPublication: true, dependencyCodes: ['convertible_bond_universe_refresh'], sourceDescription: '已发布的共享标准行情与财务标准层；本地计算', mayConsumeQuota: false, externalSources: [], retryPolicy: 'local', retryDelaysMinutes: [15, 60, 240], maxAttempts: 4 },
  ...
  { jobCode: 'hk_rate', label: '港币汇率更新', hour: 16, minute: 15, weekdays: false, marketCalendarPolicy: 'hk-rate', deadlineMinutes: 90, dataDatePolicy: 'latest_available', freshnessGate: false, freshnessMode: 'max_age', freshnessMaxAgeMinutes: 1440, sourceDescription: '港股收市报价到达后采集最终汇率；A股单开市日沿用16:15', mayConsumeQuota: true, externalSources: ['exchange-rate'], retryPolicy: 'external', retryDelaysMinutes: [15, 60], maxAttempts: 3 },
  { jobCode: 'market_close:港股', label: '港股收盘数据', hour: 16, minute: 30, weekdays: false, marketCalendarPolicy: 'hk-close', deadlineMinutes: 120, dataDatePolicy: 'same_day', freshnessGate: true, sourceDescription: '港股收市时刻后20分钟的腾讯行情接口', mayConsumeQuota: true, externalSources: ['tencent'], retryPolicy: 'external', retryDelaysMinutes: [15, 60], maxAttempts: 3 },
  { jobCode: 'nav_snapshot', label: '净值快照', hour: 16, minute: 20, weekdays: false, marketCalendarPolicy: 'nav-snapshot', deadlineMinutes: 120, dataDatePolicy: 'same_day', freshnessGate: false, retryPolicy: 'local' },
  ...
  { jobCode: 'ipo_calendar_refresh', label: '打新日历与日报', hour: 18, minute: 5, additionalSchedules: [{ hour: 19, minute: 45, mode: 'enrichment', weekdays: true }], weekdays: true, deadlineMinutes: 240, catchupWindowMinutes: 4200, catchupMode: 'latest_only', dataDatePolicy: 'latest_available', freshnessGate: true, requiresDataWatermark: false, importance: 'high', datasetDependencies: [{ datasetCode: 'ipo_history', scopeKey: 'GLOBAL', partitionDatePolicy: 'business_date', requireQualityStatus: 'passed' }, { datasetCode: 'bond_issuance_events', scopeKey: 'CN', partitionDatePolicy: 'business_date', requireQualityStatus: 'passed' }, { datasetCode: 'bond_listing_liquidity', scopeKey: 'CN', partitionDatePolicy: 'business_date', requireQualityStatus: 'passed' }], sourceDescription: '读取16:30上市前预测缺口检查后已发布的新股事实与17:30已核验的可转债流通规模事实生成日报；19:45再刷新同一基础分区对应的日报', retryPolicy: 'local' },
  { jobCode: 'convertible_bond_universe_refresh', label: '可转债行情同步（共享收盘采集）', hour: 8, minute: 0, weekdays: true, afterTradingDay: true, catchupMode: 'latest_only', dataDatePolicy: 'previous_trading_day', freshnessGate: true, strictDatasetPublication: true, datasetFailureThreshold: 3, sourceDescription: '腾讯行情与 Tushare 接口；同时发布股票日行情、估值、复权因子和停牌分区', mayConsumeQuota: true, externalSources: ['tencent', 'tushare'], retryPolicy: 'external', retryDelaysMinutes: [15, 60, 240], maxAttempts: 4, collectorRole: 'shared_cn_market_eod' },
  ...
  { jobCode: 'hk_trade_calendar_sync', label: '港交所交易日历同步', hour: 7, minute: 50, weekdays: false, calendarWeekdays: true, catchupMode: 'latest_only', requiresDataWatermark: false, sourceDescription: '港交所年度官方安排与 Tushare 增量日历，均经同一同步入口落库', mayConsumeQuota: true, externalSources: ['hkex', 'tushare'], retryPolicy: 'external', retryDelaysMinutes: [15, 60], maxAttempts: 3 },
  { jobCode: 'hk_ipo_preopen', label: '港股 IPO 盘前事实同步', hour: 8, minute: 30, weekdays: false, calendarWeekdays: true, marketCalendarPolicy: 'hk-open', catchupMode: 'latest_only', requiresDataWatermark: false, datasetDependencies: [{ datasetCode: 'hk_trade_calendar', scopeKey: 'HK', partitionDatePolicy: 'business_date' }], sourceDescription: '港交所主板/GEM 新上市页面、官方披露易、腾讯行情名称与 HKIPOx 公开认购倍数', mayConsumeQuota: true, externalSources: ['hkex', 'tencent', 'hkipox-public'], retryPolicy: 'external', retryDelaysMinutes: [15, 60], maxAttempts: 3 },
  { jobCode: 'hk_ipo_postclose', label: '港股 IPO 盘后事实同步', hour: 18, minute: 10, weekdays: false, calendarWeekdays: true, marketCalendarPolicy: 'hk-open', catchupMode: 'latest_only', requiresDataWatermark: false, datasetDependencies: [{ datasetCode: 'hk_trade_calendar', scopeKey: 'HK', partitionDatePolicy: 'business_date' }], sourceDescription: '港交所配售结果、标题检索、年度新上市报表、腾讯行情名称与事实层', mayConsumeQuota: true, externalSources: ['hkex', 'tencent'], retryPolicy: 'external', retryDelaysMinutes: [15, 60], maxAttempts: 3 },
  { jobCode: 'hk_ipo_enrichment', label: '港股 IPO 官方文件、日线与市场信号补全', hour: 19, minute: 40, mode: 'enrichment', weekdays: false, calendarWeekdays: true, marketCalendarPolicy: 'hk-open', catchupMode: 'latest_only', requiresDataWatermark: false, datasetDependencies: [{ datasetCode: 'hk_trade_calendar', scopeKey: 'HK', partitionDatePolicy: 'business_date' }], sourceDescription: '港交所年度新上市报表、官方招股书/配售结果 PDF、腾讯港股 K 线首日/五日覆盖与 HKIPOx 公开认购倍数', mayConsumeQuota: true, externalSources: ['hkex', 'tencent', 'hkipox-public'], retryPolicy: 'external', retryDelaysMinutes: [15, 60], maxAttempts: 3 },
...
const JOB_CONTRACTS = {
  'company_financial_incremental_sync': { externalApis: ['disclosure_date', 'income', 'balancesheet', 'cashflow', 'fina_indicator'], producesDatasets: ['stock_financial_reports'], consumesDatasets: ['stock_master', 'financial_disclosure_schedule'], maxExternalCallsPerRun: null },
  ...
  'market_close:A股': { externalApis: ['tencent_quote'], producesDatasets: ['account_daily_prices'], consumesDatasets: ['account_positions'], maxExternalCallsPerRun: null },
  'market_close:可转债': { externalApis: ['tencent_quote'], producesDatasets: ['account_daily_prices'], consumesDatasets: ['account_positions'], maxExternalCallsPerRun: null },
  'market_close:LOF/ETF': { externalApis: ['tencent_quote'], producesDatasets: ['account_daily_prices'], consumesDatasets: ['account_positions'], maxExternalCallsPerRun: null },
  'hk_rate': { externalApis: ['exchange_rate_realtime', 'exchange_rate'], producesDatasets: ['hk_fx_rate'], consumesDatasets: [], maxExternalCallsPerRun: null },
  'market_close:港股': { externalApis: ['tencent_quote'], producesDatasets: ['account_daily_prices'], consumesDatasets: ['account_positions'], maxExternalCallsPerRun: null },
  'nav_snapshot': { externalApis: [], producesDatasets: ['nav_snapshot'], consumesDatasets: ['account_daily_prices'], maxExternalCallsPerRun: 0 },
  'index_recent': { externalApis: ['index_daily'], producesDatasets: ['index_daily'], consumesDatasets: [], maxExternalCallsPerRun: null },
  'ipo_calendar_refresh': { externalApis: [], producesDatasets: ['ipo_calendar'], consumesDatasets: ['ipo_history', 'trade_calendar', 'bond_issuance_events', 'bond_listing_liquidity'], maxExternalCallsPerRun: 0 },
  'convertible_bond_universe_refresh': { externalApis: ['cb_basic', 'stock_basic', 'cb_daily', 'daily', 'daily_basic', 'adj_factor', 'suspend_d', 'tencent_quote'], producesDatasets: ['bond_master', 'bond_daily', 'stock_daily', 'stock_valuation', 'stock_adj_factor', 'stock_suspend_calendar'], consumesDatasets: [], maxExternalCallsPerRun: null },
  // 候选规模随上市转债变化，不能编造固定调用数；完整遍历由接口官方频率、任务超时和失败分类保护。
  'convertible_bond_revision_motive_inputs_sync': { externalApis: ['top10_cb_holders', 'pledge_stat'], producesDatasets: ['bond_motive_inputs'], consumesDatasets: ['bond_master'], maxExternalCallsPerRun: null },
  ...
  'convertible_bond_announcement_history_sync': { externalApis: ['cb_issue', 'sse', 'szse', 'cninfo'], producesDatasets: ['bond_announcement_facts', 'bond_issuance_events', 'bond_redemption_events', 'bond_listing_liquidity'], consumesDatasets: ['bond_master'], maxExternalCallsPerRun: null },
  ...
  'hk_ipo_preopen': { externalApis: ['hkex_new_listings', 'hkex_prospectus', 'tencent_quote', 'hk_ipo_public_page'], producesDatasets: ['hk_ipo_facts', 'hk_ipo_subscription_signals'], consumesDatasets: ['hk_trade_calendar'], maxExternalCallsPerRun: null },
  ...
  'hk_ipo_enrichment': { externalApis: ['hkex_official_documents', 'hkex_cancellation_announcements', 'hkex_history_report', 'tencent_hk_daily', 'tencent_quote', 'hk_ipo_public_page'], producesDatasets: ['hk_ipo_facts', 'hk_ipo_cancellation_notice', 'hk_ipo_grey_market_signals', 'hk_ipo_subscription_signals'], consumesDatasets: ['hk_trade_calendar'], maxExternalCallsPerRun: null },
