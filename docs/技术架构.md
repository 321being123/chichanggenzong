...
| 可转债公告与下修监控 | 公告事实工作日 07:40；首次全量按市场/年度窗口批量取，后续按 `ops.sync_cursors` 上次成功日期回扫 3 天重叠窗口；交易所主取失败或分页不完整才切巨潮，官方来源编号幂等去重，补发/更正覆盖原记录；启动时校验公告游标、解析积压和派生水位，必要时仅重跑库内官方 PDF 解析 | 公告事实 → 下修事件 → `reset-v2` 交易日进度；成功空结果不切备源；主备均失败时保留上一份有效结果、游标不前移；接口显式返回 `expected_trade_date`、`stale` 与公告质量诊断 |
