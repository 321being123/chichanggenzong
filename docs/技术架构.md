...
- 下修动机：`convertible_bond_revision_motive_inputs_sync` 首次同步 Tushare `top10_cb_holders` 与 `pledge_stat` 后写入持有人/质押事实，后续幂等增量保存并比较持债变动；08:15 在下修数学进度完成后执行 `motive-v1` 评分，保存五维组成项、触发成熟度、可执行性、安全性、来源引用和输入哈希。迁移 112/113/114 补齐变动前后值、清仓标记、周期日期/标签、评分范围约束和治理检索索引；评分候选沿用公开上市债券、生命周期、条款和强赎停止日口径，历史财报/持有人/质押按公告可见日隔离，正股 PB 与收盘价要求同日。列表只新增可点击“动机等级”，详情页读取 `analytics.convertible_bond_revision_motive_daily`，不临时抓取外部数据。
