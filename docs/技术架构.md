...
| 可转债下修监控（`analytics.convertible_bond_revision_latest`） | 公告同步读取巨潮/上交所/深交所并写入公告事实、`event.convertible_bond_revision_events`；`calculateConvertibleBondRevisionStatus` 按 `reset-v2` 写入 `analytics.convertible_bond_trigger_daily` | `/api/bond-revision`、可转债“下修”子页 | 仅当前交易日有行情的交易所上市公开转债；退市、未上市、定向和私募债券不进入下修范围。视图以最新交易日行情为驱动，条款、进度和公告按单券索引读取，接口在数据库完成状态、剩余天数排序和截断；公告事件按上海当前日期过滤，避免行情水位落后时隐藏已发布公告。下修计数从发行/条款生效日开始；成功下修从生效日重新计数；不下修锁定期结束后从交易日历归一化后的 `next_eligible_date` 重新计数；没有固定日期的“锁定至某次董事会召开日”保持锁定，等待后续公告。官方提议、股东大会和实施事件优先于数学计数；董事会提议超过 30 个自然日且无后续事件时回落到数学状态。剩余天数采用 `rolling-v1` 滚动观察窗口算法，按未来交易日模拟，不再用固定的“要求天数-已满足天数”代替。深交所使用上市公司公告频道并分页，上交所/深交所来源失败时可从库内官方 PDF 重新解析；不下修解析器 `v7` 支持正文内嵌不下修决定、明确日期、债券到期日、首个交易日后重新起算和董事会事件锁定。迁移 090/091/092/093/094/095/096/100 负责建表、条款重解析、提议有效期、事件回填、滚动剩余天数和视图重建；其中迁移 092 会先补齐 `minimum_future_days` 再重建视图，兼容旧生产库；条款解析器为 `terms-v3`。接口同时返回行情日、下修计算日和 `expected_trade_date`，数据落后时明确标记 `stale` | 触发价/正股收盘价/转股价：元；进度：已满足/要求交易日；剩余规模：亿元；状态：提议、股东大会待审/已通过、已实施、锁定、临近触发、跟踪、数据不完整；“数据不完整”仅表示真实缺少条款、交易日历、正股关联或行情 | 公告事实每日 07:40；行情完成后 08:15 与强赎、列表、估值串行计算；启动时检查公告游标和下修派生结果水位，落后时按游标重叠窗口补公告并只重算本地派生数据；交易日历、正股停牌、重新起算日和转股价变动均写入诊断 |
...
| 可转债公告与下修监控 | 公告事实工作日 07:40；首次全量，后续按 `ops.sync_cursors` 上次成功日期回扫 3 天重叠窗口；官方来源编号幂等去重，补发/更正覆盖原记录；启动时校验公告游标和派生水位，必要时补跑 | 公告事实 → 下修事件 → `reset-v2` 交易日进度；失败保留上一份有效结果，游标不前移；接口显式返回 `expected_trade_date` 与 `stale` |
