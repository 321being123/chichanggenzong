...
备份专项复核（2026-09-05）：目前仅本机加密文件校验和本机临时库恢复有成功证据，异地同步未启用。异地服务缺少 `REQUIRE_ENCRYPTION=1`；恢复脚本仅检查用户对象数量大于零并输出迁移数量，未做关键业务数据对账；告警依赖业务数据库先入库，数据库不可用时没有独立邮件兜底，成功恢复也未自动关闭旧告警。因此整项验收未通过，不能把已有成功日志解释为完整灾备闭环，详见安全审计报告第 9 节。
