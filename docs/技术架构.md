...
- AI：交易截图和非结构化 Excel 识别使用 Agnes AI 的 OpenAI 兼容 `POST /v1/chat/completions`；官方已废弃 `agnes-2.0-flash`，当前默认 `agnes-2.5-flash`，消息与图片 `image_url` 格式保持兼容。`VISION_API_URL/VISION_API_KEY/VISION_MODEL`、允许的模型和主机白名单由配置控制；旧模型配置由迁移 155 幂等改为 `agnes-2.5-flash`，旧环境变量不会覆盖当前默认模型。所有 AI 请求经 `fetchSafeAi` 手动跟随跳转；每一跳重新解析全部 DNS 地址，拒绝私网/回环地址，限制同源且最多 3 跳，并将连接固定到已验证公网 IP。固定 IP 请求使用与 dispatcher 同一 npm `undici` 实例的 `fetch`，避免与 Node 内置 `fetch` 版本不一致导致请求在发出前失败。
