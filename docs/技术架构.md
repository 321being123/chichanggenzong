...
| 可转债下修监控与动机评分（`analytics.convertible_bond_revision_latest` / `analytics.convertible_bond_revision_motive_daily`） | 公告同步读取巨潮/上交所/深交所并写入公告事实、`event.convertible_bond_revision_events`；`calculateConvertibleBondRevisionStatus` 按 `reset-v2` 写入触发进度；`convertibleBondRevisionMotiveService` 读取财务、持有人、质押、控制人、行情和安全性事实，按 `motive-v1.1` 写入每日评分快照及 `cycle-v1` 周期；`server/scripts/backtestConvertibleBondMotive.js` 按公告前时点生成回测结果到 `analytics.convertible_bond_revision_motive_backtests` | `/api/bond-revision`、`/api/bond-revision/:tsCode/motive-detail`、可转债“下修”子页和单债详情页 | 下修数学口径沿用现有统一视图；动机评分只对数据库已入库事实计算，五维满分 100（历史25/偿债压力30/转股稀释20/治理15/市场10），但“剩余规模低于发行规模”不再加分，其他转债/同行下修仅作市场背景不计入单债动机分；部分输入缺失或历史样本外回测未通过时保留原始研究分和明细，但不输出“动机偏弱/存在动机”等预测等级，只有输入完整且模型完成校准才允许分档；已提议、待表决、已通过、已实施等状态由页面展示事实，不再当作预测结果。`input_snapshot`、`source_refs`、`input_hash` 固化本次计算事实；详情页不展示来源引用，接口仍保留来源留痕供审计；回测未达门槛时必须留痕并保持 `unavailable` | 价格/转股价/底价：元；规模/财务：元；持有人数量：张；比例：百分比点；评分：分；完整率：0-1；状态：研究评分、可执行性和数据质量 | 持有人/质押输入同步任务收盘日次日 07:20（首次全量、后续按已入库快照幂等更新）；公告事实 07:40；行情完成后 08:15 与强赎、下修数学进度、列表、动机评分、估值串行计算；详情页始终读取指定交易日评分快照。模型规则变化必须升级 `model_version`；历史回填、公告前样本外回测与生产同步需按生产流程单独授权 |
...
- 下修动机：`convertible_bond_revision_motive_inputs_sync` 按当前行情候选过滤，首次同步 Tushare `top10_cb_holders` 与 `pledge_stat` 后写入持有人/质押事实，后续幂等增量保存并比较持债变动；两类接口独立调用，按运行时返回的权限/网络结果分别记录游标错误，任一接口失败都不阻断另一类事实写入，也不覆盖已有有效事实；单券或单公司的空结果只跳过该对象，接口限速、额度、权限或熔断等接口级错误才停止当前批次；缺失事实优先，已尝试但为空的对象按 `last_attempt_at` 后置，达到批次调用上限或接口停止后未实际请求的对象不推进游标，并返回 `partial` 供下一批继续，避免补数批次反复卡在同一批，同时保留后续复查机会。任务按 `afterTradingDay` 运行并保留 72 小时补偿窗口，避免周末盘后数据错过周一补偿。08:15 在下修数学进度完成后执行 `motive-v1.1` 评分，保存五维组成项、触发成熟度、可执行性、安全性、来源引用和输入哈希；无效的发行规模比较和跨债种市场热度不计入单债动机分，部分输入缺失时不输出预测等级，已确定下修状态展示事实。迁移 112/113/114 补齐变动前后值、清仓标记、周期日期/标签、评分范围约束和治理检索索引；评分候选沿用公开上市债券、生命周期、条款和强赎停止日口径，历史财报/持有人/质押按公告可见日隔离，正股 PB 与收盘价要求同日。列表只新增可点击“动机等级”，详情页读取 `analytics.convertible_bond_revision_motive_daily`，不临时抓取外部数据。
