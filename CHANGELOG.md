...
## 2026-07-27 · 0.3.1.1
- 文章打开速度优化：内容立刻用快速方式显示出来，更精细的排版在后台完成后无感替换，不再出现打开文章要等一两秒的情况。
- 可转债周期图表的纵坐标刻度改为整数（且都是5的倍数，如100、120、140），不再出现小数刻度。
- 「可转债周期」标题的悬停说明只在鼠标移到文字上时弹出，移到标题旁边的空白处不再误弹。
...
- 四条历史曲线合并在一张图里用颜色区分，每条曲线有自己颜色的纵坐标轴，点指标按钮可随时显示或隐藏某条曲线；曲线做了平滑处理，更接近收益率曲线的观感。
- 打开页面默认显示全部时间范围的周期分位和综合估值两条曲线；鼠标移到图上会弹出当天各指标的具体数值，弹框紧跟鼠标。
- 指标说明不再固定占一块位置，鼠标移到「可转债周期」标题上会浮出说明。
- 图表下方新增历史数据明细表，按日期倒序每页显示50条，翻页控件与别处一致。
- 投资笔记评论发送限制放宽：5分钟内可发10条（原来只能发3条，正常连续回复会被拦）。
- 修复文章内容偶尔一直转圈不显示的问题：加载超过2秒自动切换备用方式显示，不再卡住。

## 2026-07-26 · 0.3.0.3
- 登录页新增“忘记密码”，账号绑定邮箱通过验证码验证后才能重置密码。
- 重置验证码有效期为5分钟，并限制发送频率和错误尝试次数。

## 2026-07-26 · 0.3.0.2
- 分类名称和文章大纲标题过长被省略时，鼠标悬停会用统一的小弹框显示完整文字。

## 2026-07-26 · 0.3.0.1
- 修复微信公众号文章链接无法导入的问题，支持直接提取微信文章标题、正文和图片。

## 2026-07-26 · 0.2.0.23
- 修复可转债安全性列表中个别正股名称乱码的问题，名称优先使用腾讯行情中正确解码的结果。

## 2026-07-26 · 0.2.0.22
- 新增「投资笔记」模块：支持文章分类、草稿与发布、公开分享、登录评论，以及链接、Markdown、文本和 Word 文件导入。
- 编辑器使用本地 Vditor 资源，补齐工具说明、图片上传、表格行列选择和表格增删行列浮框等常用编辑能力。
- 新增投资笔记写作权限和后台管理，文章、分类及评论操作按登录身份和作者归属进行校验。
- 加强内容安全：Markdown 安全渲染、危险链接拦截、导入地址防内网访问、图片类型与大小限制。
- 首页新增投资笔记入口和最新文章展示，文章标题可直接进入正文。

## 2026-07-25 · 0.2.0.21
- 后台新增「大模型配置」页面：可以手动配置用来识别图片和 Excel 的 AI 模型（填名称、模型名、接口地址、密钥），密钥在页面上自动打码隐藏，不用担心泄露；这也是为了之前模型偶尔失效时，能直接在后台换模型救急。
- 每个模型都有「测试」按钮：点一下后台就发一次真实小请求，马上告诉你通不通、花了多久，状态栏用绿/红灯实时显示每个模型最近一次能不能用、什么时候用的。
- 支持配置多个模型做兜底：识别图片或表格时按顺序自动调用，上一个失效就自动切到下一个，整个过程你无感知；排在最前面的是默认模型，可以「设默认」或上下调整顺序。
- 第一次启动会自动把原来写在服务器配置里的模型搬进后台，老功能照常工作，不用手动迁移。
- 后台「定时任务」页面现在每个任务卡片鼠标放上去会显示这条任务是干什么的；最近执行记录表里也新增了「说明」列，直接写清楚每条任务的作用。
