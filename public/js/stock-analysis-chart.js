(function () {
  function renderStabilityChart(s, selectedRange) {
    ...
    if(!rows.length){el.innerHTML=controls()+summary()+'<div class="stock-analysis-chart-empty">所选时间内暂无年度数据</div>'+historyTable();if(window.BusinessTable)window.BusinessTable.attach(el,{page:'#main-stock-analysis',top:'.nav',sticky:true});bindFilters();return;}
    ...
    var oldTooltip=document.querySelector('.stock-analysis-chart-tooltip[data-stock-analysis-tooltip]');if(oldTooltip)oldTooltip.remove();el.innerHTML=controls()+summary()+'<div class="stock-analysis-chart">'+legend+'<svg viewBox="0 0 '+W+' '+H+'" style="width:100%;min-width:'+W+'px" role="img" aria-label="盈利与分红稳定性图表">'+svg.join('')+'</svg><div class="stock-analysis-chart-tooltip app-tooltip" data-stock-analysis-tooltip="1"></div></div>'+historyTable();if(window.BusinessTable)window.BusinessTable.attach(el,{page:'#main-stock-analysis',top:'.nav',sticky:true});bindFilters();
    ...
    Array.prototype.forEach.call(el.querySelectorAll('.year-hit'),function(hit){ChartInteraction.bind(hit,tooltip,function(){tooltip.innerHTML=tooltipHtml(rows[Number(hit.getAttribute('data-year-index'))]);tooltip.style.display='block';},function(){tooltip.style.display='none';});});
  }
