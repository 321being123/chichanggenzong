...
| 可转债强赎公告同步 | 收盘日次日 07:45 | 从巨潮同步提前赎回、不提前赎回、赎回实施、到期兑付暨摘牌与赎回结果公告，标准化写入 `event.convertible_bond_call_events`，供强赎监控页、股债分析和估值读取；最终公开视图保留到期 30 日内的 `maturity_near` 状态（口径见本节末“收盘日次日”） |
