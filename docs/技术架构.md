...
外部请求预算计数写入 PostgreSQL `ops.external_call_budgets`，Token＋接口熔断写入 `ops.external_circuits`，数据集并发锁使用 PostgreSQL advisory lock；三者职责分离，由 Node 服务和打新日报/新股历史 Python 子进程共用。Python Guard 默认开启，只有开发/测试显式 `EXTERNAL_CALL_GUARD=0` 才可关闭，生产环境即使设置 0 也强制开启；两端在同一预算表和 `external_budget:{source}` 锁下计数，并使用探测所有者、一次性令牌和5分钟租约回收异常退出的半开探测。内部预算到限使用 `BUDGET_WAIT` 等待下一分钟/交易日，不切备用 Token、不写 Token 熔断；真正上游 429/额度错误仍按接口级熔断和主备策略处理。业务错误保留 `api_name` 进入任务编排层；公告同步失败只记录 `last_error`，已有成功游标由数据库单调递增保护，不会因重叠窗口、旧进程收尾或并发写入回退。过期的 `job_runs` 租约由 Worker 自动回收，活动运行每分钟续租。
